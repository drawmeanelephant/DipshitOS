...
- The fleet is discovered, not listed (`tools/gate/fleet.sh`). The generated
  `docs/gate-fleet-inventory.md` is **gitignored — never commit it.** Render
  locally with `bash tools/inventory-gates.sh` if you want to read it.
  `--check` asserts zero orphans, locale-invariant render, and spec-shape
  guards; it does not require a tracked snapshot. The path stays exempt from
  the coordination gate so leftover `Touches` declarations are a no-op.
...
- **One editor per file at a time.** The gate fails when two ACTIVE claims from
  ...
  (`docs/gate-fleet-inventory.md`) are exempt — that path is gitignored and
