...
#
# Usage: make-image.sh [EFI_BINARY] [IMAGE_PATH]
...
# from scratch every time. Fails loudly if a required tool or the compiled
# EFI application is missing.
...
python3 "$SCRIPT_DIR/mkfat32.py" --size-mb "$SIZE_MB" "$IMAGE" "$EFI_BIN" \
