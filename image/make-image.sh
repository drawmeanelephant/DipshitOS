...
# M33 SB6 (claim 6864): SB6WM.BIN + SB6OLD.BIN + SB6NEW.BIN are flat DSK1
# programs (the perf-payoff before/after behind
...
if [ -f "$SB6WM_BIN" ]; then
    if [ "$(head -c 4 "$SB6WM_BIN")" != "DSK1" ]; then
        fail "'$SB6WM_BIN' does not start with the 'DSK1' image magic -- run 'zig build' first."
    fi
...
if [ -f "$SB6OLD_BIN" ]; then
    if [ "$(head -c 4 "$SB6OLD_BIN")" != "DSK1" ]; then
        fail "'$SB6OLD_BIN' does not start with the 'DSK1' image magic -- run 'zig build' first."
    fi
...
if [ -f "$SB6NEW_BIN" ]; then
    if [ "$(head -c 4 "$SB6NEW_BIN")" != "DSK1" ]; then
        fail "'$SB6NEW_BIN' does not start with the 'DSK1' image magic -- run 'zig build' first."
    fi
    SB6NEW_ARGS+=("$SB6NEW_BIN")
fi
