...
TMRTEST_BIN="${27:-$ROOT_DIR/zig-out/bin/TMRTEST.BIN}"
...
TMRTEST_ARGS=()
if [ -f "$TMRTEST_BIN" ]; then
    if [ "$(head -c 4 "$TMRTEST_BIN")" != "DSK1" ]; then
        fail "'$TMRTEST_BIN' does not start with the 'DSK1' image magic -- run 'zig build' first (it produces zig-out/bin/TMRTEST.BIN)."
    fi
    TMRTEST_ARGS+=("$TMRTEST_BIN")
fi
...
if [ -f "$TMRTEST_BIN" ]; then
    printf '%s\n' "$LISTING" | grep -q 'TMRTEST.BIN' || fail "TMRTEST.BIN missing from the image listing"
fi
