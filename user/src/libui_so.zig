...
pub export fn ui_draw_char_8x8(wid: u32, char: u8, px: u32, py: u32, fg: u32) void {
    if (char < 0x20 or char > 0x7e) return;
    const glyph = font8x8.glyphs[char - 0x20];
    var row: u32 = 0;
    while (row < 8) : (row += 1) {
        ...
        while (col < 8) : (col += 1) {
            if ((bits & (@as(u8, 1) << @intCast(col))) != 0) {
                _ = ui_win_fill(wid, px + col, py + row, 1, 1, fg);
            }
        }
    }
}
...
pub export fn ui_draw_string(wid: u32, text: [*:0]const u8, x: u32, y: u32, fg: u32) void {
    ...
    var cur_x = x;
    for (str) |c| {
        ui_draw_char_8x8(wid, c, cur_x, y, fg);
        cur_x += 8;
    }
}
...
pub export fn ui_draw_button(wid: u32, text: [*:0]const u8, x: u32, y: u32, w: u32, h: u32, hovered: bool, pressed: bool) void {
    ...
    _ = ui_win_fill(wid, x, y, w, h, bg_color);
    _ = ui_win_fill(wid, x, y, w, 1, ui.theme_border());
    _ = ui_win_fill(wid, x, y + h - 1, w, 1, ui.theme_border());
    _ = ui_win_fill(wid, x, y, 1, h, ui.theme_border());
    _ = ui_win_fill(wid, x + w - 1, y, 1, h, ui.theme_border());

    const str = std.mem.sliceTo(text, 0);
    const text_w = @as(u32, @intCast(str.len)) * 8;
    const tx = if (w > text_w) x + (w - text_w) / 2 else x + 2;
    const ty = if (h > 8) y + (h - 8) / 2 else y + 2;
    ui_draw_string(wid, text, tx, ty, ui.theme_text_primary());
}
