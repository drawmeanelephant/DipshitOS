...
#      PLUS 5 (the counter's 1 text + 2 user-stack + 2 EL1-stack pages
...
GATE_LOG="artifacts/live-kill-gate.txt"
exec > >(tee "$GATE_LOG") 2>&1
trap 'sleep 0.5' EXIT

BOOTS="${BOOTS:-1}"
REPORT="artifacts/live-kill-report.txt"
...
run_one() {
    local tag="$1"
    local run_log="artifacts/live-kill-run-$tag.txt"
    local serial_copy="artifacts/live-kill-serial-$tag.log"
    ...
    if [ -f artifacts/vm-serial.log ]; then
        ...
        markers="$(grep -aFc -- "$COUNTER_MARKER" artifacts/vm-serial.log || true)"
        local kill_line
        kill_line="$(grep -anF -- "kill: COUNTER.BIN armed" artifacts/vm-serial.log | head -1 | cut -d: -f1 || true)"
        if [ -n "$kill_line" ]; then
            local before after
            before="$(grep -anF -- "$COUNTER_MARKER" artifacts/vm-serial.log | awk -F: -v k="$kill_line" '$1 < k' | wc -l | tr -d ' ')"
            after="$(grep -anF -- "$COUNTER_MARKER" artifacts/vm-serial.log | awk -F: -v k="$kill_line" '$1 > k' | wc -l | tr -d ' ')"
        ...
        [ "$(grep -aFc -- "tasks user-exec exited status=137" artifacts/vm-serial.log || true)" -ge 1 ] && killed_exit=1
        [ "$(grep -aFc -- "procs COUNTER.BIN exited status=137" artifacts/vm-serial.log || true)" -ge 1 ] && procs_killed_exit=1
        [ "$(grep -aFc -- "tasks user-exec reaped" artifacts/vm-serial.log || true)" -ge 1 ] && killed_reap=1
        grep -a -qE -- "name=COUNTER.BIN state=exited task=reaped .*exit=137" artifacts/vm-serial.log && killed_row=1
        # 4. Page recovery: phase-3 free = phase-1 free + 5.
        ...
        if [ -n "$p1" ] && [ -n "$p3" ]; then
            [ $((16#$p3)) -eq $((16#$p1 + 5)) ] && page_recovery=1 || page_recovery=0
        fi
        ...
        counter_row="$(grep -aE -- "procs: id=[0-9]+ name=COUNTER.BIN state=running" artifacts/vm-serial.log | head -1 || true)"
        counter_tid="$(printf '%s\n' "$counter_row" | sed -E 's/.*task=([0-9]+).*/\1/' || true)"
        user_row="$(grep -aE -- "procs: id=[0-9]+ name=USER.BIN state=running" artifacts/vm-serial.log | head -1 || true)"
        user_tid="$(printf '%s\n' "$user_row" | sed -E 's/.*task=([0-9]+).*/\1/' || true)"
        if [ -n "$counter_tid" ] && [ -n "$user_tid" ]; then
            [ "$counter_tid" = "$user_tid" ] && slot_reused=1 || slot_reused=0
        fi
        [ "$(grep -aFc -- "exec: loaded USER.BIN size=" artifacts/vm-serial.log || true)" -ge 1 ] && reexec_landed=1
        # 6. Shell responsive; no exception park.
        [ "$(grep -aFxc -- "rx-kill-phase1" artifacts/vm-serial.log || true)" = 1 ] && phase1_echo=1
        [ "$(grep -aFxc -- "rx-kill-killed" artifacts/vm-serial.log || true)" = 1 ] && kill_echo=1
        [ "$(grep -aFxc -- "rx-kill-ok" artifacts/vm-serial.log || true)" = 1 ] && echo_ok=1
