...

{
    ...
    echo "assertions: winmove open/fill/present/move/raise/get/query/hide/show/loop markers (winmove: get 1024,528,256,192 through slot 18 + winmove: query 1024,528,256,192 z=2 focused=1 visible=1 dirty=1 through slot 19 + hide ok/show ok through slot 20), dui: windows=3 + dui[2]: user user rect=1024,528,256,192 owner=<pid> visible=1, syscalls implemented=40 with open=1/fill=4/present=3/move=2/raise=1/get=1/query=1/set_visible=2/close=0, dui move 2 1024 528 + dui raise 2 (EL1h), the HIDDEN capture shows no red/cyan/white blocks at the clamped spot (the window disappeared), and the LATEST capture shows red+cyan+white blocks at the NEW spot + dark-blue background dominant + no terminal foreground inside the new rect + the old spot free of the colored blocks (the window returned)"
