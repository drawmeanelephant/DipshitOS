...
# now prints rows 0-33 — implemented=34, rows 9/10/11 with calls > 0)
...
if [ -f "$SERIAL" ]; then
    ...
    # 0-33 (implemented=34) and the seam rows show real call counts.
...

{
    ...
    echo "phase 2: syscalls (implemented=34, rows 9/10/11 with calls > 0) + net udp / net counters (rx=2, tx=2, loop=1, drop=0)"
