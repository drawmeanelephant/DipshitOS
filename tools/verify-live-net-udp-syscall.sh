...
# now prints rows 0-41 — implemented=42, rows 9/10/11 with calls > 0)
...
if [ -f "$SERIAL" ]; then
    ...
    # 0-41 (implemented=42) and the seam rows show real call counts.
    grep -a -qF -- "syscalls: slots=64 implemented=42" "$SERIAL" && SYSCOUNT=1
...

{
    ...
    echo "phase 2: syscalls (implemented=42, rows 9/10/11 with calls > 0) + net udp / net counters (rx=2, tx=2, loop=1, drop=0)"
