#!/usr/bin/env bash
#
# build-goview.sh -- build the M71h Go image viewer (user/go/view) with the
# GOOS=virelai fork toolchain, the same way tools/go/build-files.sh builds the
# file manager. Output: .build/go/GOVIEW.ELF
#
# GOVIEW.ELF replaces Zig VIEW.BIN (M36 IMG5, #826), so it is a host-share ELF
# like every other Go app, not an in-image Zig program.
#
# Usage: bash tools/go/build-goview.sh
#
