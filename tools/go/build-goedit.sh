#!/usr/bin/env bash
#
# build-goedit.sh -- build the M58b Go editor (user/go/edit) with the
# GOOS=virelai fork toolchain, the same way tools/go/build-tabapp.sh builds
# the M58a file manager. Output: .build/go/GOEDIT.ELF
#
# Usage: bash tools/go/build-goedit.sh
#
