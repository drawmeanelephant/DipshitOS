#!/usr/bin/env bash
#
# build-tabapp.sh -- build the M56d demo Go tab app (user/go/tabapp/demo) with
# the GOOS=virelai fork toolchain, the same way tools/go/build-web.sh builds
# the browser. Output: .build/go/GOTABAPP.ELF
#
# Usage: bash tools/go/build-tabapp.sh
#
