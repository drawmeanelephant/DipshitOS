#!/usr/bin/env bash
#
# build-note.sh -- build the M66c Go notepad (user/go/note) with the
# GOOS=virelai fork toolchain, the same way tools/go/build-gocalc.sh builds the
# calculator. Output: .build/go/NOTE.ELF
#
# Usage: bash tools/go/build-note.sh
#
