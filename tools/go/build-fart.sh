#!/usr/bin/env bash
#
# build-fart.sh -- build the M58f Go sound app (user/go/fart) with the
# GOOS=virelai fork toolchain, the same way tools/go/build-files.sh builds the
# file manager. Output: .build/go/FART.ELF
#
# Usage: bash tools/go/build-fart.sh
#
