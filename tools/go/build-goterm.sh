#!/usr/bin/env bash
#
# build-goterm.sh -- build the M58c Go terminal front-end (user/go/term) with
# the GOOS=virelai fork toolchain, the same way tools/go/build-goedit.sh
# builds the M58b editor. Output: .build/go/GOTERM.ELF
#
# Usage: bash tools/go/build-goterm.sh
#
