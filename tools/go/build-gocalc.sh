#!/usr/bin/env bash
#
# build-gocalc.sh -- build the Go calculator (user/go/calc) with the
# GOOS=virelai fork toolchain, the same way tools/go/build-goedit.sh builds
# the M58b editor. Output: .build/go/GOCALC.ELF
#
# Usage: bash tools/go/build-gocalc.sh
#
