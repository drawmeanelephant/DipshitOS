#!/usr/bin/env bash
#
# build-gitui.sh -- build M74b's in-guest Git TUI (GOGITUI.ELF) with the
...
if ! grep -q "bubbletea/v2 ${VERSION}" "$REPO/user/go/gitui/THIRD-PARTY.txt"; then
...
WORK_DIR="$REPO/.build/gitui-work"
...
OVERLAY="$REPO/.build/gitui-overlay.json"
...
TEMP_MOD="$REPO/user/go/.gitui.mod"
...
log "building virelai/gitui with Bubble Tea ${VERSION} -> $OUT"
( cd "$REPO/user/go" &&
    GOOS=virelai GOARCH=arm64 go build -mod=mod -modfile "$TEMP_MOD" -overlay "$OVERLAY" \
        -ldflags "-s -w" -o "$OUT" ./gitui )
