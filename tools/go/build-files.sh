#!/usr/bin/env bash
#
# build-files.sh -- build GOFILES.ELF, the M74a (issue #1644) Charm file
# manager (user/go/files), with the GOOS=virelai fork. Charm stays in the Go
# module cache — staged and overlaid like build-charmhello.sh/build-pulse.sh,
# never vendored in-tree. Output: .build/go/GOFILES.ELF
#
...
# user/go/files/THIRD-PARTY.txt (sources stay in the module cache, so the
...
if ! grep -q "bubbletea/v2 ${VERSION}" "$REPO/user/go/files/THIRD-PARTY.txt"; then
...
# directory).
WORK_DIR="$REPO/.build/files-work"
...
# overlay build-charmhello.sh and build-pulse.sh use (tools/go/overlay/charm).
OVERLAY="$REPO/.build/files-overlay.json"
...
TEMP_MOD="$REPO/user/go/.files.mod"
...
log "building virelai/files with Bubble Tea ${VERSION} -> $OUT"
( cd "$REPO/user/go" &&
    GOOS=virelai GOARCH=arm64 go build -mod=mod -modfile "$TEMP_MOD" -overlay "$OVERLAY" \
        -ldflags "-s -w" -o "$OUT" ./files )
