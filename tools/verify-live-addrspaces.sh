...
#   1. The EL0 payload still runs at its USER VAs (0x400000 text / 0x80000000
#      stack) under the user root, survives the SVC sequence, and the
#      `syscalls`/`uaccess` evidence lines are unchanged — the whole EL0
#      boundary works on the new address space.
...
run_one() {
    ...
    if [ -f artifacts/vm-serial.log ]; then
        ...
        # print_hex emits "0x" + fixed 16 lowercase digits: text_va / stack_va.
        [ "$(grep -aFc -- "addrspaces: user text=0x0000000000400000 stack=0x0000000080000000" artifacts/vm-serial.log || true)" -ge 1 ] && user_va=1
    ...
    echo "$tag: runner-rc=$rc serial-bytes=$bytes banner=$banner write=$write_ok marker=$marker write-count=$write_count mon=$mon userspace=$userspace_ok echo=$echo_ok t0sz=$t0sz user-va=$user_va el0dev0=$el0dev0 el0ok=$el0ok leaves=$leaves_ok shell-root=$shell_root worker-root=$worker_root user-own-root=$user_own fatal=$fatal" | tee -a "$REPORT"
    [ "$rc" = 0 ] && [ "$banner" = 1 ] && [ "$write_ok" = 1 ] && \
        [ "$marker" = 1 ] && [ "$write_count" = 1 ] && \
        [ "$mon" = 1 ] && [ "$userspace_ok" = 1 ] && [ "$echo_ok" = 1 ] && \
        [ "$t0sz" = 1 ] && [ "$user_va" = 1 ] && [ "$el0dev0" = 1 ] && \
