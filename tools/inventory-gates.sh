...
#
# Usage:
#   bash tools/inventory-gates.sh          # write docs/gate-fleet-inventory.md
#                                          # locally (gitignored; never commit)
#   bash tools/inventory-gates.sh --check  # registration + spec-shape guards:
#                                          # fail on orphans, on a tracked
#                                          # snapshot, on locale-sensitive
#                                          # render, on spec-order / spec-count
#                                          # violations. Does NOT require a
#                                          # committed markdown file — that
#                                          # snapshot was a merge-conflict
#                                          # factory (every spec/tooling PR
#                                          # rewrote the same table).
...
# can also split a multi-byte sequence and write invalid UTF-8 into the
# generated markdown. `--check` additionally asserts render equality across two
...
EOF

    {
        ...
        echo "> issue #934). **Do not commit** -- gitignored. Render locally"
        echo "> when you want to read it. \`--check\` asserts zero orphans and"
        echo "> spec-shape; it does not require a tracked snapshot. Subdirectory"
        echo "> tooling (\`lib/\`, \`status/\`, \`context/\`, \`gate/\`) is"
        echo "> summarized below, not rowed."
        echo
        echo "## Summary"
        echo
        echo "| metric | count |"
        ...
        echo "> edits. Discovery lives in tools/gate/fleet.sh, not this file."
        echo
    ...
    LAST_ORPHAN_COUNT=$orphans
    LAST_ORPHAN_LIST="$orphan_list"
    rm -f "${fleet_tsv:-}"
}
...
if [ "$CHECK" -eq 1 ]; then
    ...
    render_to "$tmp"
    rc=0
    # Registration guard without a merge participant: fail if the generated
    # snapshot is tracked (that is the PR #1348 conflict class — every
    # spec/tooling PR rewrote the same table) or if a gate-class script is
    # registered nowhere. Locale invariance and spec-shape stay below.
    if git rev-parse --is-inside-work-tree >/dev/null 2>&1 && \
       git ls-files --error-unmatch "$REPORT" >/dev/null 2>&1; then
        echo "inventory-gates --check: FAIL -- $REPORT is tracked in git."
        echo "It is a generated snapshot of the whole tools/ tree (line counts,"
        echo "fleet list, directory file counts). Committing it makes every"
        echo "spec/tooling PR conflict. git rm --cached it; it is gitignored."
        echo "Render locally with 'bash tools/inventory-gates.sh' if you want to read it."
        rc=1
    else
        echo "inventory-gates --check: OK ($REPORT is not a tracked merge participant)"
    fi
    if [ "$LAST_ORPHAN_COUNT" -ne 0 ]; then
        echo "inventory-gates --check: FAIL -- $LAST_ORPHAN_COUNT orphan gate-class script(s)" \
             "(no just recipe, no fleet membership, no CI name, no status.md row):"
        printf '%s' "$LAST_ORPHAN_LIST" | sed '/^$/d; s/^/  /'
        rc=1
    else
        echo "inventory-gates --check: OK (0 orphan gate-class scripts)"
    fi
