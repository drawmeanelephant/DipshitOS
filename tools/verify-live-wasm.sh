...
echo "=== verify-live-wasm: M35 W2+W3+W4 — C hello-world, wasm window app, wasm file app + wasm float app in-guest, $BOOTS boot(s) ==="
zig version
...
# Script1: the three short non-display apps run first (floatapp, fileapp,
# hello — pure compute/write, proven safe concurrently; the "faulted once"
# three-way was a display-app overlap), then the winapp loads and spins;
# FLOATAPP first keeps the winapp's overlap set at <=1 app. `w3: win ok`
...
run_one() {
    ...
    local bytes=0 banner=0 hello=0 exit55=0 heck0=0 winid=0 winok=0 win21=0 drow=0 file512=0 fileecho=0 float590=0 float1=0 float2=0 float3=0 float4=0 float5=0 heck4=0 ok=0 fatal=0 blitok=0
    if [ -f "$SER" ]; then
        bytes="$(wc -c < "$SER" | tr -d ' ')"
    ...
    if [ $banner -eq 1 ] && [ $hello -eq 1 ] && [ $exit55 -eq 1 ] && [ $heck0 -eq 1 ] && \
       ...
       [ $heck4 -eq 1 ] && [ $ok -eq 1 ] && [ $blitok -eq 1 ] && [ $fatal -eq 0 ] && [ $rc -eq 0 ]; then
        PASS=1
    fi
...
EOF
    ...
    if [ $PASS -eq 1 ] && [ "${REDOK:-0}" -eq 1 ]; then
        echo "PASS $tag: hello (write byte-exact, exit 55) + wasm window app (dui row rect=100,100,96,48, blits>=1 post-raise, red fill block in the marker capture) + wasm file app (FILE.TXT echoed byte-exact, exit 512) + wasm float app (FLOATAPP.WASM executes in-guest: five float opcode families byte-exact — f64 arith/mC, f64 bit hex, trunc_sat, f32 path, i64.extend — exit 590) — virelai.h contract proven live" | tee -a "$REPORT"
        return 0
    fi
