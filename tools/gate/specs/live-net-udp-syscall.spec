...
# `implemented=` is the LIVE count of registered rows (76 since #1228), so any
# literal here rots on every slot landing: this spec pinned 68 and went red on
# #1214/#1228 without anything noticing (the CI shards skip). Assert the
# report's shape plus the slot rows below (the claim-5069 shape-not-count
# precedent, live-ls-l.spec); tools/inventory-gates.sh --check now fails a
# pinned one (#1345).
