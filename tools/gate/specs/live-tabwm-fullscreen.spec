...
# (`calc: resize relayout`), so its `dui`/`echo` round trip is the LAST
# thing before --script-expect. Previously script3 fired on `calc: ready`
# and raced `calc: resize relayout`: whichever landed second could fall
# outside the 1.5s expect tail and flake the run. Gating script3 on the
# relayout (and the expect on script3's own echo) removes the race.
vgate_run A -- --screen '$RUN_DIR/screen' --script '$RUN_DIR/script-A.txt' --script2 '$RUN_DIR/script2-A.txt' --script2-after 'tabwm: sidebar-rendered' --script3 '$RUN_DIR/script3-A.txt' --script3-after 'calc: resize relayout' --script-expect 'rx-m42-ok' --timeout 90
...
# Same #1063 fix: script3's `procs`+echo now runs after the relayout (its
# old `tabwm: launch` trigger fired before CALC had even opened, so the
# expect stopped the VM early); the expect waits on its own echo.
vgate_run B -- --screen '$RUN_DIR/screen' --script '$RUN_DIR/script-B.txt' --script2 '$RUN_DIR/script2-B.txt' --script2-after 'tabwm: sidebar-rendered' --script3 '$RUN_DIR/script3-B.txt' --script3-after 'calc: resize relayout' --script-expect 'rx-m42-b2-ok' --timeout 120 --via-virtio --input-chords 'ctrl-space,6,4,-,b,i,t,return' --input-chords-after 'tabwm: sidebar-rendered'
