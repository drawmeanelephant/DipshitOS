# live-usb-msc.spec -- M43 U2 (issue #1033): USB Mass Storage Bulk-Only
# Transport + minimal SCSI over the U1 bulk engine and the runner --usb-msd
# flag. `usb msc probe` runs TEST UNIT READY -> INQUIRY -> READ CAPACITY(10)
# -> a deterministic 512-byte sector write/read-back, printing the wire's
# honest answer for every field (the [observed] rows for
# docs/hardware-contract.md). The disk image is the U1 image (an 8 MiB
# MBR + hand-built FAT32 volume + an LBA-1 pattern) because VZ's sniffer
# refuses a bare/zeroed image; the probe writes to LBA 100, a gap sector.

vgate_name live-usb-msc "M43 U2: USB MSC BOT + minimal SCSI probe over --usb-msd"
vgate_share seed
...
# Bulk-only boot (no --input): the probe consumes the MSD through the U1
# engine; no HID device is attached, so `input: armed` must be absent.
vgate_run 01 -- --script '$RUN_DIR/script.txt' --script-after "virelai>" --usb-msd '$RUN_DIR/msd.img' --timeout 120

vgate_assert 01 serial-contains 'VirelaiOS kernel has seized control.'
# BOT/SCSI probe: TUR + INQUIRY + READ CAPACITY + a write/read-back. The
# gate pins the SHAPE + the image-determined geometry; the exact INQUIRY
# strings are the [observed] contract rows (they are VZ-version data, not
# gate-stable — assert the `vendor=` prefix, record the value in the doc).
vgate_assert 01 serial-contains 'usb msc: dev slot=1 tur_status=0 tur_ok=1 tur_stage=none'
vgate_assert 01 serial-contains 'usb msc: inquiry ok=1 vendor='
vgate_assert 01 serial-contains 'usb msc: capacity ok=1 last_lba=16383 block_len=512'
vgate_assert 01 serial-contains 'usb msc: rw lba=100'
vgate_assert 01 serial-contains 'rx-usb-msc'
# The write path is the card's proof: a sector written and read back
# byte-exact (diff=0). If VZ's MSD refuses writes this assert fails loudly
# and the card re-scopes on the honest observation instead of faking it.
vgate_assert 01 serial-contains 'write_ok=1 read_ok=1 diff=0 ok=1'
vgate_assert 01 serial-absent 'usb msc: no bulk-capable device'
