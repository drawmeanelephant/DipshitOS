...
#   2. `exec NOTEPAD.BIN`          -> notepad: open id=2, the tab-aware declaration,
#                                  the full-viewport relayout (notepad: ready).
#   3. POINTER INJECTION after "notepad: ready": one click at (158, 70) - the
...
#   5. NOTEPAD observes WIN_CLOSE ("notepad: win_close"), exits cleanly
#      ("notepad: exiting 43"); script3 (after the exit marker) reads the
...
vgate_run 01 -- --screen '$RUN_DIR/screen' --via-virtio \
    ...
    --pointer-virtio '158,70,c' --pointer-virtio-after 'notepad: ready' \
    --script3 '$RUN_DIR/script3.txt' --script3-after 'notepad: exiting 43' \
