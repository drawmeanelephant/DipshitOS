...
# hands the raw console to the shell seat (`login: shell=sh -> GOSH.ELF
# serial`) -- which attaches the serial front-end, adopts the persisted prompt,
# and runs a typed command. The untouched default (boot 1) stays the monitor;
# the existing live-sh* gates prove the monitor default on a clean share.
#
# The setting VALUE is still `sh`: it names the shell seat, not the binary, so
# a persisted share does not need a schema migration.
#
# HOST PREREQ: bash tools/go/build-gosh.sh -> .build/go/GOSH.ELF

vgate_name live-shell-default "#1084 SH8 default-shell flip: shell=sh boots into GOSH.ELF, default stays the monitor"
vgate_share seed
...
# Boot 2 = shell=sh: the login handed the console to the shell seat.
...
# Boot 1 is the monitor; boot 2 is the shell seat. Prove the ORDER: the
# monitor-side settings write precedes the login handoff, which precedes the
# shell's ready marker.
i_settings = ser.find("settings: shell=sh (persisted)")
i_login = ser.find("login: shell=sh -> GOSH.ELF serial")
...
# The shell adopted the persisted prompt (it appears after its ready marker).
assert "SH8PROMPT" in ser[i_ready:], "the login shell did not adopt the settings prompt"
