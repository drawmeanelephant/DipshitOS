...
vgate_run 01 -- --net '$RUN_DIR/cap.bin' --net-arp-respond 10.0.0.2 \
    ...
    --script '$RUN_DIR/script.txt' --script-expect 'rx-gonet-ok' --timeout 120
...
vgate_assert 01 serial-contains 'gonet: connected 10.0.0.2:8080'
...
vgate_assert 01 serial-contains 'rx-gonet-ok'
vgate_assert 01 output-contains "NET-TCP: answered the guest's HTTP request with 200 OK"
vgate_assert 01 serial-absent '[EXC] parking:'

vgate_assert 02 serial-contains 'gonet: connected 10.0.0.2:8080'
...
after = [i for i, l in enumerate(lines) if l.startswith("gonet: hb=") and i > fail]
if not after:
    sys.exit("FAIL: the heartbeat did NOT continue after the read failed closed")
before = [i for i, l in enumerate(lines) if l.startswith("gonet: hb=") and i < fail]
if not before:
