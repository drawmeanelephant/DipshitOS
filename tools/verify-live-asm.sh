...
run_one() {
    local tag="$1"
    local run_log="$(art live-asm-run-$tag.txt)"
    local serial_copy="$(art live-asm-serial-$tag.log)"
    ...
    local rc=$?
    set -e
    [ -f "$RUN_DIR/vm-serial-$tag.log" ] && cp "$RUN_DIR/vm-serial-$tag.log" "$(art live-asm-serial-$tag.log)" || true
    ...
    if [ -f "$SER" ]; then
        ...
        [ "$(grep -aFxc -- "rx-asm-ok" "$SER" || true)" = 1 ] && echo_ok=1
...
: > "$REPORT"
{
    echo "VIRELAIOS live assembler gate (M22 D2, issue #325, claim 9815)"
    echo "revision: $REVISION branch=$BRANCH boots=$BOOTS dirty-files=$DIRTY"
