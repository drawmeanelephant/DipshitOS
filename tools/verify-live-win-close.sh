...

{
    ...
    echo "assertions: win: close ok x2, win: open id=2 x2 (never id=3), procs WINCLOSE.BIN exited status=88 x2, dui: windows=2 (window gone) + no dui[2]: row, syscalls implemented=40 with open=1/close=1 in the pre-re-exec snapshot"
