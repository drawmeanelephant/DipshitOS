...
MODULES=(alloc arp console csprng dhcp esp exec exceptions fat gic handoff ipv4 lineedit machine mailbox memmap mmu monitor nvram_console process scheduler shell syscall tcp timer tokenizer uaccess udp userspace virtio_blk virtio_custom virtio_entropy virtio_net)
