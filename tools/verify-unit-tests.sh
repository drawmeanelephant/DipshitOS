...
MODULES=(alloc arp console csprng dhcp driving_award esp events exec exceptions fat font8x8 gic handoff input ipv4 lineedit machine mailbox memmap mmu monitor nvram_console process road_pops scheduler settings shell syscall tcp text timer tokenizer uaccess udp userspace virtio_blk virtio_custom virtio_entropy virtio_gpu virtio_net xhci)
