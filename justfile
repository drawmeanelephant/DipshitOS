...
# go-stress / go-panic / go-win / live-smp-scale class-B gates:
# `just verify-vz` includes them, and they refuse
# to run (honestly) until this has produced .build/go/{GOHELLO,GOARGS,
# GOROUT,GOSTRESS,GOPANIC,GOWIN,GOSCALE}.ELF on this machine. First run takes several minutes
# (one Go make.bash pass; the second cross-std pass is phase-2 opt-in via
# GOVIRELAI_STD=1).
go-toolchain:
    bash tools/go/build-go.sh tools/go/hello.go tools/go/goargs.go tools/go/goroutines.go tools/go/gostress.go tools/go/gopanic.go tools/go/gowin.go tools/go/gonet.go tools/go/smpscale.go
