...
test "shell: mock-fed end-to-end session produces the exact transcript" {
    ...
    const expected =
        ...
        "  wm          M32 WMS2/WMS4 render-server register: the registered WM server pid, present-sequence counter, presents, COMPOSITE_TICK count, and SET_WINDOW chrome submissions ('wm none' means the shell idle shim is compositing)\n" ++
