...
pub fn decode_keyboard_report(rep: []const u8) void {
    ...
    for (keys) |k| {
        // M32 WMS5 Gate 2 (claim 9850): while a WM owns input, the raw
...
test "input: WMS5 Gate 2 — keyboard geometry chords gate off and the raw key fans out when a WM owns input (claim 9850)" {
