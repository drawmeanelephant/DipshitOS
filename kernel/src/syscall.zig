    ...
    switch (cmd) {
        ...
        wm_server.wmctl_set_window => {
            ...
            if (have_geom) {
                ...
                // M32 WMS5 Gate 2 (claim 9850): apply with LAYOUT semantics
        ...
        wm_server.wmctl_set_state => {
            // M32 WMS5 Gate 2 (issue #625, claim 9850): the visibility /
...
test "syscall: SET_STATE (cmd 4, claim 9850) applies visibility/workspace/ws-switch with the seam refusals" {
    userspace.init();
    init(test_writer);
    wm_server.init();
    _ = scheduler.init();
