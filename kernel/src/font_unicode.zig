...
pub fn glyph(cp: u21) ?*const [8]u8 {
    ...
    if (cp >= 0x2500 and cp <= 0x257F) return box_glyph(cp);
    return null;
}

/// M73a-2 (#1631): the box-drawing frame set a Charm/Lipgloss border
/// actually uses — light, heavy, double, and rounded strokes — added to
/// THIS table (the single M20 lookup), never a second one. Light
/// horizontals ride row 4 and verticals column 4 (heavy: rows 3-4 /
/// columns 3-4; double: rows 2-5 / columns 2-5), so adjacent cells join
/// seamlessly. Box runes outside this set stay null and paint through
/// the caller's U+FFFD fallback.
fn box_glyph(cp: u21) ?*const [8]u8 {
    return switch (cp) {
        // Light.
        0x2500 => &.{ 0x00, 0x00, 0x00, 0x00, 0xff, 0x00, 0x00, 0x00 }, // ─
        0x2502 => &.{ 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10 }, // │
        0x250c => &.{ 0x00, 0x00, 0x00, 0x00, 0xf0, 0x10, 0x10, 0x10 }, // ┌
        0x2510 => &.{ 0x00, 0x00, 0x00, 0x00, 0x0f, 0x10, 0x10, 0x10 }, // ┐
        0x2514 => &.{ 0x10, 0x10, 0x10, 0x10, 0xf0, 0x00, 0x00, 0x00 }, // └
        0x2518 => &.{ 0x10, 0x10, 0x10, 0x10, 0x0f, 0x00, 0x00, 0x00 }, // ┘
        0x251c => &.{ 0x10, 0x10, 0x10, 0x10, 0xf0, 0x10, 0x10, 0x10 }, // ├
        0x2524 => &.{ 0x10, 0x10, 0x10, 0x10, 0x1f, 0x10, 0x10, 0x10 }, // ┤
        0x252c => &.{ 0x00, 0x00, 0x00, 0x00, 0xff, 0x10, 0x10, 0x10 }, // ┬
        0x2534 => &.{ 0x10, 0x10, 0x10, 0x10, 0xff, 0x00, 0x00, 0x00 }, // ┴
        0x253c => &.{ 0x10, 0x10, 0x10, 0x10, 0xff, 0x10, 0x10, 0x10 }, // ┼
        // Heavy.
        ...
        0x254f => &.{ 0x18, 0x18, 0x18, 0xff, 0xff, 0x18, 0x18, 0x18 }, // ┿
        // Double.
        0x2550 => &.{ 0x00, 0x00, 0xff, 0x00, 0x00, 0xff, 0x00, 0x00 }, // ═
        0x2551 => &.{ 0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x24, 0x24 }, // ║
        0x2554 => &.{ 0x00, 0x00, 0xfc, 0x04, 0x04, 0xe4, 0x24, 0x24 }, // ╔
        0x2557 => &.{ 0x00, 0x00, 0x3f, 0x20, 0x20, 0x27, 0x24, 0x24 }, // ╗
        0x255a => &.{ 0x24, 0x24, 0xe4, 0x04, 0x04, 0xfc, 0x00, 0x00 }, // ╚
        0x255d => &.{ 0x24, 0x24, 0x27, 0x20, 0x20, 0x3f, 0x00, 0x00 }, // ╝
        0x2560 => &.{ 0x24, 0x24, 0xe4, 0x24, 0x24, 0xe4, 0x24, 0x24 }, // ╠
        0x2563 => &.{ 0x24, 0x24, 0x27, 0x24, 0x24, 0x27, 0x24, 0x24 }, // ╣
        ...
        // Rounded (v1: the light-corner silhouette; typographic rounding
        // is future font polish, not a mapping gap).
        0x256d => &.{ 0x00, 0x00, 0x00, 0x00, 0xf0, 0x10, 0x10, 0x10 }, // ╭
        0x256e => &.{ 0x00, 0x00, 0x00, 0x00, 0x0f, 0x10, 0x10, 0x10 }, // ╮
        0x256f => &.{ 0x10, 0x10, 0x10, 0x10, 0x0f, 0x00, 0x00, 0x00 }, // ╯
        0x2570 => &.{ 0x10, 0x10, 0x10, 0x10, 0xf0, 0x00, 0x00, 0x00 }, // ╰
        else => null,
    };
}

test "font_unicode: the box frame set maps to real art, never FFFD (M73a-2 #1631)" {
    const frame_set = [_]u21{
        0x2500, 0x2502, 0x250c, 0x2510, 0x2514, 0x2518, 0x251c, 0x2524, 0x252c, 0x2534, 0x253c,
        0x2501, 0x2503, 0x250f, 0x2513, 0x2517, 0x251b, 0x2523, 0x252b, 0x2533, 0x253b, 0x254f,
    ...
    for (frame_set) |cp| {
        ...
        try std.testing.expect(!std.mem.eql(u8, g.?, &fffd_glyph));
        var lit: usize = 0;
        for (g.?) |row| lit += @popCount(row);
        try std.testing.expect(lit >= 4); // a stroke, not an empty cell
    }
    // Unmapped box runes stay null — the CALLER's fallback paints them.
    try std.testing.expect(glyph(0x2571) == null);
    try std.testing.expect(glyph(0x2572) == null);
}
