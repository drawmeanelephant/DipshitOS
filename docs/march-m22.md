...
| D5 | **System call tracer.** `strace cmd` — wraps exec, logs every syscall invocation with name, args, and return value to the serial console. Each syscall prints: `[strace] sys_name(args) = retval`. | ✅ | `artifacts/live-strace-report.txt` — verify-live-strace PASS 1/1 on VZ: `[strace 1] sys_write(0x1, 0x400020, 0x1a) = 0x1a` and `sys_exit(0x2a) = —` for a HELLO.ELF run; `strace off` disarms | No new syscall slot (ABI budget unchanged): monitor command `strace exec <file> [args]` arms `syscall.strace_pid` around the exec — the loader records the pid at the success point so every subsequent syscall from that process traces. dispatch() prints synchronously (`[strace <pid>] <name>(x0, x1, x2) = res`, hex); sys_exit is special-cased pre-call with an em-dash result since its handler never returns. Registry 55 (now 57 with font). |
| D6 | **Process viewer.** `ps` table + windowed PS.BIN — `ps` prints PID/NAME/STATE/MEM/CPU/TASK; PS.BIN renders the same snapshot in a 300×200 window refreshed at 1 Hz via `sys_timer_set`. | ✅ | `artifacts/live-ps-report.txt` — verify-live-ps PASS 1/1 on VZ with --display --input: `ps` printed the table twice and windowed PS.BIN launched (`ps: ready`). `ps` reuses the existing `sys_procs` (slot 7) snapshot; windowed viewer polls it via the app-timer. | Monitor registry 57 (now with strace/sym/font); ProcessInfo now exposes `cpu_usage`. |
| D7 | **Environment viewer.** `printenv` dumps the shell's KEY=VALUE table (M18 T12). `export VAR=val` / `export VAR` already existed; `printenv` reads it via shell accessors. | ✅ | `artifacts/live-printenv-report.txt` — verify-live-printenv PASS 1/1 on VZ: a script `export`s `D7VAR=m22-lane-d` then `printenv` shows `D7VAR=m22-lane-d`. | Monitor registry 58; shell `env_pair_count`/`env_pair` accessors; `sh` script runner already handles `export`. |
