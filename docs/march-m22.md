...
| D9 | **System information dashboard** — extended `sysinfo` (cpu/memory/allocator/storage free-total/network/graphics/input/uptime sections) | ✅ | `verify-live-sysinfo.sh` PASS 1/1 — all sections incl. storage `free=0x…/0x…` post-`mount esp` | KNOWN GAP: on a FRESH boot the storage section omits free=/total — `fat.geometry()` reads empty until an explicit `mount esp`, even though ls/stat/find work off the boot window. Needs a follow-up issue. |
