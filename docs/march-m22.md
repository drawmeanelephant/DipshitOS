...
| D14 | **Developer console** — DEVCONS.BIN split-screen log pane + command prompt | ✅* | `verify-live-devcons.sh` PASS 1/1 — loaded, window open, `devcons: ready`, no faults, screenshot | *Typed-input proof deferred while issue #179 (synthesized keyboard seam reports events=0) is open. FIXED live bug: the flat DSK1 build data-aborted on its first BSS write (`far=0x400928`) before `ready`; now SEGMENTED DSK3 (claim 5220). |
