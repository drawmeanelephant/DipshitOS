...
|:-----|:------|:------|:-----------|:-------|:--------|:------|
| **UI1** | [#926](https://github.com/drawmeanelephant/DipshitOS/issues/926) **Modularize `ui.zig` with Facade Pattern** | cleanup | — | 🔄 claimed | `user/src/lib/ui.zig`, `user/src/lib/ui/*` | Split 5k-line `ui.zig` into `abi.zig`, `theme.zig`, `draw.zig`, `widgets/`. Root `ui.zig` re-exports all symbols via `pub usingnamespace`. 33 apps + `LIBUI.SO` compile untouched. |
| **UI2** | [#927](https://github.com/drawmeanelephant/DipshitOS/issues/927) **Anti-Aliased Rounded Rectangles & Sidebar Tokens** | visual | UI1 (#926) | 🔄 queued | `user/src/lib/ui/draw.zig`, `user/src/lib/ui/theme.zig` | `fill_rounded_rect` with smooth anti-aliased corner masks. Sidebar tokens (`sidebar_w=180`, `tab_row_h=38`, active/hover pill colors). Inter typography metrics. |
