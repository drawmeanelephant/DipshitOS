...
| **SX5** | [#986](https://github.com/drawmeanelephant/DipshitOS/issues/986) **Primary-manager completion** | manager | SX1–SX4 | ⬜ open | — | TABWM god-menu overlay (launch from the manifest — the sidebar button currently only emits `tabwm: god-menu`), default-manager seam (the default VM stays shim-only — the load-bearing gate invariant), dedicated Class-B live gate, docs/status rows for the whole arc. |
