...
| DQ2 | **Tab-bar chrome render** (#840) — WM strip, truncation, active/hover/× | render | DQ1-stable | 🔄 claim 6562 — code in review, pixel proof pending | `kernel/src/wnd_core.zig`, `kernel/src/driving_award.zig`, `kernel/src/syscall.zig`, WND policy bit, `user/src/tabhold.zig`, `tools/verify-live-tabstrip.sh` | Design: kernel-blits-WM-decided (tooltip precedent), kind bit only, no new slot. Unit green everywhere; attach path live-proven; strip snapshot blocked on infra flake (#843). |
| DQ3 | **Tab mouse interaction** (#839) — click switch, × close, drag-detach | interact | DQ2 | 🔄 claim 8605 — code in review, click+× live-proven, drag pending | `user/src/wnd.zig` only + `tools/verify-live-tabclick.sh` | Pure hit-test + press/drag state + pointer wiring; unit 105/105; click 4× + × 1× live; full green pending quiet host (#843). |
