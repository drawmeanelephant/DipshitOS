...
> `go-args`, `go-goroutines`, `go-stress`) are NOT hermetic — they exec
> `.build/go/GOHELLO.ELF` + `.build/go/GOARGS.ELF` + `.build/go/GOROUT.ELF`
> + `.build/go/GOSTRESS.ELF`, and refuse to run (honestly, with the build
> hint) until `just go-toolchain` has provisioned this machine (it builds
> ALL FOUR fixtures). The recipe is idempotent; the first run takes several
