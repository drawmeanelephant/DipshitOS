...
8215's landed EL0/SVC evidence, honest not-yet-claimed statuses for the seven
remaining cards, and a serialized agent split that encodes both file ownership
and shared-host VZ-run contention. No kernel, host, tool, status, roadmap, gate
inventory, or live-gate file changed. `zig fmt --check` passed and the final
