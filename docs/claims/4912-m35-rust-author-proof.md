...
- **Touches:** tests/virelai.zig (new), tests/nl.rs (new), user/src/wasm-corpus/nl.wasm (new pinned binary), user/src/wasm.zig (host test: rustc-built fixture executes byte-exact through the capture seam), tools/verify-live-wasm.sh (cross-language phase), docs/wasm-import-contract.md (§7 author recipes — virelai.zig + rustc — made real; C recipe corrected), docs/wasm-core-scoping.md (post-M35 note), docs/status.md (post-M35 note), claim + branch log; claim id via `bash tools/status/claim-id.sh`
...
- **Status:** 🔄 agent/buffy2/m35-rust-author-proof
...
Claim id derivation: `bash tools/status/claim-id.sh "agent/buffy2/m35-rust-author-proof" "rust-author-proof"` → **4912**.
