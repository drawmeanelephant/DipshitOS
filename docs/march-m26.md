...
| N13 | **Network-aware application status.** Apps check connection and IP status before network operations. | ⬜ | — | Not landed: neither `fetch.zig` nor `ping.zig` introspects link/IP state before operating (no `net_ready`/`ip_set` consumer in userland). They fail fast through bounded connect/send errors instead (see N14). |
| N14 | **Offline state handling.** Graceful offline handling without hanging or unbounded waiting. | 🔶 | `user/src/fetch.zig`, `user/src/ping.zig` (M5-bounded paths) | Fail-fast behavior exists via the M5 bounded connect/send error returns (no hangs observed), but it is generic timeout handling, not explicit offline detection. True offline-first handling needs the N13 introspection seam first. |
