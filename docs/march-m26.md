...
| N13 | **Network-aware application status.** Apps check connection and IP status before network operations. | ✅ done 2026-08-28 — claim 8852 | `user/src/lib/netstatus.zig`, `user/src/ping.zig`, `user/src/fetch.zig` | PING.BIN/FETCH.BIN preflight via one `sys_net_stats` (slot 62) snapshot before the first network op; offline (no IP) and no-route (ARP miss) exit fast with distinct statuses (2/3, 3/4). Class-B gate `tools/verify-live-net-offline.sh` PASS 23/23 (offline boot ×2 apps, no-route boot, online control — N1/N3 shapes unaffected). |
