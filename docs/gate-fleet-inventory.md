...
> issue #934). **Do not hand-edit.** Re-render locally to read it.
> **Do not commit this file in a spec or script PR** -- parallel
> PRs rewrote the same table and GitHub blocked the merge. The
> fleet source of truth is `tools/gate/fleet.sh list`. `--check`
> enforces spec-order and locale invariance, not snapshot equality.
> Subdirectory tooling (`lib/`, `status/`, `context/`,
> `gate/`) is summarized below, not rowed.
...
> edits. Do not commit a re-render of this snapshot in that PR.
...
| spec | `go-calc` | 1 run / 15 assert | go-calc.spec -- Go calculator (CALC.BIN successor, issue #1378) class-B |
...
| spec | `go-git` | 1 run / 21 assert | go-git.spec -- issue #1337: GOTGIT.ELF clones a tiny fixture repo over |
...
| spec | `live-sh-monitor` | 1 run / 9 assert | live-sh-monitor.spec -- M49 card SD1 class-B gate (issue #1128, ADR 0021 D1). |
| spec | `live-sh-profile` | 1 run / 9 assert | live-sh-profile.spec -- M49 card SD2 class-B gate (issue #1129). |
| spec | `live-sh-tools` | 1 run / 16 assert | live-sh-tools.spec -- M49 card SD3 class-B gate (issue #1130). |
...
| spec | `live-ssh-endpoint` | 1 run / 22 assert | live-ssh-endpoint.spec -- M51 SSH5 (#1172, ADR 0025 D8) class-B positive. |
| spec | `live-ssh-negative` | 3 run / 17 assert | live-ssh-negative.spec -- M51 SSH5 (#1172, ADR 0025 D8) class-B negatives |
| spec | `live-ssh-nocred` | 1 run / 7 assert | live-ssh-nocred.spec -- M51 SSH5 (#1172, ADR 0025 D8) class-B negative: |
...
| spec | `live-web` | 12 run / 98 assert | live-web.spec -- WEB.ELF: the in-guest Go browser (Go app shell + the |
| spec | `live-web-ttf` | 3 run / 46 assert | live-web-ttf.spec -- issue #1346 (class B): Go EL0 paints REAL TrueType. |
...
| `inventory-gates.sh` | 579 | tooling | n | n | y | n | inventory-gates.sh -- regenerate (or --check) the machine-generated gate |
