...
| P4 | **Shell functions.** `fn name { cmd1; cmd2 }` — user-defined multi-command sequences. `name` executes them. Bounded: 16 functions × 16 commands each. `fn` lists defined functions. `fn -d name` deletes. | ✅ | claim 7033 P4: host tests pass (640/640 shell, 6 new fn tests); live gate pending | `shell.zig` BSS table (16 × (64 name + 16 × 256 body) = ~66 KiB). Functions stored as parsed command strings, executed sequentially via `shell_handle_expanded`. Bare `fn` lists; `fn -d` deletes. No return values, no arguments (P8). |
