...
| U6 | **First-boot experience.** `welcome` (alias `tour`) walks a scripted tour; `about` is refreshed to explain the system in one screen; a boot motd/status line summarizes what booted and what works (deterministic). | ✅ done (claim 8323) | `kernel/src/monitor.zig` (refreshed `about`, `welcome`/`tour`, `motd`), `kernel/src/shell.zig`, `tests/transcript-console.txt` fixture, `tools/verify-transcript.sh` PASS | Gate: transcript — the tour/welcome walk, refreshed `about`, and deterministic `motd` line are asserted; transcript gate `tools/verify-transcript.sh` and live help gate `tools/verify-live-help.sh` PASS. Depends on U0. |
| U7 | **`sysinfo`.** One command prints the support snapshot: version, machine, memory, pages, processes, networking, graphics, input status. Stable fields byte-deterministic so a support report is reproducible. | ✅ done (claim 2990) | `kernel/src/monitor.zig` (`cmd_sysinfo`), `kernel/src/shell.zig`, `tests/transcript-console.txt`, `tools/verify-transcript.sh` PASS | Gate: transcript — the stable fields are asserted byte-exact; session-dynamic counters are asserted as present. Depends on U0. |
| U8 | **Persistent settings.** A small `settings` surface (`settings get/set <key> <value>`) backed by a config file on the DATA partition (the live-gfs pattern): hostname, theme accent, prompt string, scrollback size. Read at boot, persists across reboot. | ✅ done (claim 2649) | `kernel/src/settings.zig`, `kernel/src/monitor.zig` (`cmd_settings`), `kernel/src/main.zig`, `kernel/src/shell.zig`, `tools/verify-live-settings.sh` PASS (1/1) | Gate: live persistence across reboot (`tools/verify-live-settings.sh`) — `settings set hostname elephant-box` + `settings set prompt elephant>` persists to `SETTINGS.TXT` on the DATA partition and restores dynamically upon fresh reboot under VZ. Depends on U0. |
