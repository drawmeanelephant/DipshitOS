...
Continuous Integration runs on GitHub Actions using a dedicated macOS job (`build-and-test-macos` on `macos-latest`). There are **no Linux runners and no QEMU anywhere in CI** — the only host runtime, Apple Virtualization.framework, runs on macOS:
...
- **Quality Checks**: Validates Zig formatting (`zig fmt --check`), runs
  the kernel-monitor unit tests (`bash tools/verify-unit-tests.sh`), the
  automated console transcript test (`zig build test-console`), the
  multiagent coordination gate (`bash tools/verify-coordination.sh`), and
  project snapshot generation (`zig build context`).

### 2. Mandatory Pull Requests & Branch Protection

Direct pushes to the default branch (`main`) are restricted. All contributions (from humans and agents alike) must be submitted via Pull Requests that pass the required status check
(`Build (macOS + Swift Launcher)` — the job name in
...
The CI matrix originally included a Linux job that cross-compiled the guest
and (in its first form) executed headless QEMU. That job and all `run-qemu`
steps were removed — the macOS job is the entire pipeline, matching the
project's Apple silicon / Virtualization.framework-only identity.
...
- Every commit on `main` remains verified by automated macOS builds, tests, and binary inspection.
