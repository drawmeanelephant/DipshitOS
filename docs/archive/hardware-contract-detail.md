# Hardware contract — archived detail (pre-trim verbatim copy)

> **Archived from `docs/hardware-contract.md` on 2026-08-21 (issue #270).**
> The live [`docs/hardware-contract.md`](../hardware-contract.md) keeps only the
> device summary table + actionable facts; this file preserves the full
> per-device discovery narratives (the claim-by-claim histories behind each
> fact) verbatim so nothing was lost in the trim. **Do not edit this file**
> for new hardware findings — record those in the live contract; treat this
> as a frozen snapshot (git history: `8980e8d` is the last commit that
> touched it as the live file).
