...
- [x] At least ten commands work (27 commands, host-tested; the registry grew 20 → 21 with claim 5844's `pci`, → 22 with claim 5275's `tasks`, → 23 with claim 3594's `syscalls`, → 24 with claim 6120's `uaccess`, → 25 with claim 5804's `addrspaces`, → 26 with claim 6729's `spawn`, → 27 with claim 6783's `exec`; real command output observed post-exit via the NVRAM channel, claim 0015, and live post-MMU via claim 1517).
