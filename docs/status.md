...
| 70 | Moonshots (#1437) | M70g G2 (#1459): `--console-tcp` hardened — explicit `busy` refusal of a second client, 10 s pre-auth deadline, 1024-byte line bound, output-stall drop + keepalive, Ctrl-C cancel of a mid-line client's partial line, `--console-tcp-secret-file` (secret off argv), constant-time MAC compare; ADR 0022 D2 amended; negatives pinned as `live-console-tcp` run 03 + `live-remote-console` run 03. G1 (in-guest sshd, blocked on M68 #1436) and G3 (restore beyond same-process) unclaimed. | 🔄 2026-09-18 |
