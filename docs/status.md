...
- **Go runtime port — `GOOS=virelai`** (claim #1163 → [ADR 0025](decisions/0026-go-runtime-port.md)) — 🚧 **phase 0a in flight 2026-09-12** (single-thread first target; threads/futex = phase 0b, fault delivery = 0c). A maintained fork of the gc toolchain (the Fuchsia/TamaGo pattern — upstream has no third-party-GOOS mechanism; #35956 declined bare-metal, #73608's GOOSPKG overlay is still open). Fork source of truth lives in-repo: `tools/go/` (5 GOOS-gated overlay files + 6 declarative edits via `apply.sh`; 1.27.1 distribution base). Kernel phase 0a landed: `elf.zig` takes up to 3 PT_LOADs with a new GAP layout (segments at page-aligned declared vaddrs — the Go linker's [R+X][R][RW] shape) beside the original contiguous contract, `exec.zig` maps gap images at declared vaddrs via the aperture machinery and stages up to 1 MiB, `process.zig` caps lifted (16 mmap regions, 4096 recorded demand pages), `sys_mmap` per-call cap 1 GiB, `sys_getrandom` (the M51 SSH-P1 #1166 slot 72 — this port consumes it), CPACR_EL1.FPEN armed at boot. Memory = the runtime's sbrk platform over demand-backed `sys_mmap` (contiguity via address hints); nanotime = CNTPCT_EL0 scaled by a boot-time multiplier (EL0-readable counter, no clock syscall); no signals, locks from `lock_sema.go`, sysmon fork-gated off until slots 72/73. Class-B gate `go-hello` (`tools/gate/specs/go-hello.spec`) execs `.build/go/GOHELLO.ELF` (machine prerequisite: `bash tools/go/build-go.sh` — the fork is not a repo artifact) and asserts console + 1 MiB heap growth + a full GC cycle on serial — **PASS 1/1 on VZ 2026-09-12**, clean-room (fresh distribution → apply.sh → toolchain → binary → gate) verified end-to-end; class-A `zig build`/unit tests/BSS budget/fmt/inventory all green. Bring-up lessons are in [ADR 0025](decisions/0026-go-runtime-port.md)'s bring-up record: the runtime needs multi-M semantics from `gcenable` onward (five proc.go deltas behind one `canCreateM` const), prints must stage through the data segment (uaccess does not know the sbrk heap), and the 1.21 MiB of PT_LOAD memory drove the 2 MiB staging bound (ADR 0013 D3.1 amended, budget 13.0 MiB).
