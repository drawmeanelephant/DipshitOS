...
| 60 | Starve Zig EL0 (#1297) | No new `user/src/*.zig` apps (ADR 0030). Leftovers deleted: `EDIT.BIN` → `GOEDIT.ELF` (gate `go-edit`); `FILE.BIN` → `GOFILES.ELF` (gate `go-files`, #1374); `CALC.BIN` → `GOCALC.ELF` (gate `go-calc`, #1378, deleted M62h / #1406). `TABWM.BIN` is **retained by decision** as the Zig fallback (M68c #1451, ADR 0034) — not a leftover: `wm=tabwm` is a schema-v2 settings value, and gate `go-wm-default` 4/4 boots it (run 02). Further Zig EL0 deletions are later cards: `SH.BIN` (#1450), `NOTEPAD.BIN` (#1485). TLS stays `FETCHS.BIN` / `lib/tls`; SSH stays `SSH.BIN`. | 🔄 2026-09-19 (policy + EDIT/FILE/CALC deletes; TABWM retained — ADR 0034) |
