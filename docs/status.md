...
| Four — real randomness | Virtio entropy driver + ChaCha20 CSPRNG (RFC 7539), boot-time seed, `random` command, ASLR | 🚧 **active** — milestone-four card 1 landed 2026-08-10 (claim 2665, see the march tracker [`docs/march-m4.md`](march-m4.md) row 1): the kernel now has a REAL randomness source. The virtio-pci entropy driver (`kernel/src/virtio_entropy.zig`, DID 0x1044) re-arms post-MMU (**observed: VZ resets the device at ExitBootServices — `entropy: pre-rearm st=00`**, the claim-6420 lesson confirmed for entropy) and delivers real bytes; the freestanding ChaCha20 CSPRNG (`kernel/src/csprng.zig`) is keyed from a 64-byte boot seed (`entropy: seeded n=64`); the `random [n]` monitor command (registry 27→28) prints hex from the seeded stream; and the exec path consumes the seed for EL0 user-stack ASLR (the loaded program's stack VA is randomized per boot — `exec: loaded … stack=0x…`). New class-B gate `bash tools/verify-live-entropy.sh` **PASS 2/2** — two boots produced DIFFERENT `random 32` sequences and different exec stack placements (evidence `artifacts/live-entropy-*`, `artifacts/m4-entropy-live.txt`); all shared-seam live regressions green. |
...
- [`march-m4.md`](march-m4.md) — milestone-four per-card tracker and best-agent split (card 1 — entropy/CSPRNG — landed 2026-08-10, claim 2665; general filesystem / process abstraction / network sketched as ⬜).
