...
| 69 | Daily driver floor | Go-seat dogfood + screenshot→site honesty + chrome cohesion + intentional typefaces + edit→build→run loop (not full self-host) + GOSH daily-bar gaps; index #1527 closed. | ✅ 2026-09-20 |
...
| 72 | Charm TUI | Window tty Charm can speak (SGR/CUP/EL/ED/alt screen), `load_max` not charging BSS, a tiny Bubble Tea hello whose proof is a 2560×1440 scanout, host VHS of that window; index #1578; cards #1579–#1582. Not a vendor dump. | 🔄 proposed |
...
| **M72 — Charm TUI** | Window grid is `u8` + CSI `2J`/`H`; Charm-sized Go images die on `load_max` summing BSS | #1578 |
