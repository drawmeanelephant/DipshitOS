...
| Eight — usability: human interface (ADR 0008) | One command grammar + grouped `help` (D1), a real line editor (history, cursor, Ctrl chords, tab completion — D2), one `error:`/`usage:`/`unknown command` shape (D3), a visible-focus window model (D4), an `about`/`welcome`/`motd`/`sysinfo` support surface (D5), all enforced by gates (D6). Normative contract: [`docs/decisions/0008-human-interface-guidelines.md`](decisions/0008-human-interface-guidelines.md) | 🚧 **active** — U0 (ADR 0008, claim 8938) ✅ 2026-08-14; U1 help/catalog (claim 3275) ✅ 2026-08-14; U2 shell editing/history (claim 1809, + the latent XHCI interrupt-ring wrap fix) ✅ 2026-08-14; U3 error/usage contract (claim 1511, + the u4 history-width fix the fuzz found) ✅ 2026-08-14. U5 window HIG (claim 0935 — focus ring + title bars; gate `tools/verify-live-win-hig.sh` PASS 8/8 on VZ) ✅ 2026-08-14; U4 pointer focus + cursor (claim 4993) ⛔ blocked at the live seam — the guest side is host-tested but five synthesized pointer delivery routes each produced zero guest pointer reports; **follow-ons: (claim 9015) the real-mouse path is a class-C gate** `bash tools/verify-pointer-manual.sh` (a human at the mouse), and **(claim 3692) the CG route is a class-B gate** `bash tools/verify-live-pointer-cg.sh` that self-gates on Accessibility trust (the terminal holds none today; the runner now reports `PTR-TRUST: untrusted` honestly) and becomes fully automatable once the one-time grant exists. U6–U8 (first-boot experience, `sysinfo`, persistent settings) are the next cards — per-card tracker [`docs/march-m8.md`](march-m8.md). |
...
| Eleven — desktop platform & GUI apps (ADR 0011) | A recognizable graphical desktop for userland applications: normative ADR 0011 UI contract (A0), the zero-heap micro-widget toolkit `user/src/lib/ui.zig` + `font8x8.zig` (A1), and four consumer GUI apps — `CALC.BIN` calculator (A2), `NOTEPAD.BIN` editor with persistent `/data/notes.txt` load/save (A3), `TOP.BIN` graphical process monitor with click-to-kill (A4), and the `DESKTOP.BIN` launcher capstone (A5) | ✅ **done 2026-08-15 (cards A0–A5 live)** — ADR 0011 desktop & GUI contract (claim 0664), micro-widget toolkit `user/src/lib/ui.zig` + `user/src/lib/font8x8.zig` with 0 heap allocation / pure static BSS (claim 8155), graphical calculator `CALC.BIN` (claim 8401), graphical text editor `NOTEPAD.BIN` (claim 3234), graphical process monitor `TOP.BIN` (claim 0680), and desktop launcher `DESKTOP.BIN` (claim 2427) with class-B capstone live gate `tools/verify-live-desktop.sh` **PASS 1/1 on VZ** — CALC.BIN OK, NOTEPAD.BIN OK, TOP.BIN OK, DESKTOP.BIN OK. See [`docs/march-m11.md`](march-m11.md). |
...
17. **Milestone six, card G1 — virtio-gpu transport + framebuffer (DONE
    ...
    **Card G5 (claim 1543) DONE 2026-08-13 — Driving Award, the window
    ...
    window 0 and a 1 Hz clock overlay window 1; `win`/`win focus`/`win
    raise`/`win hit` (registry 39→40); gate `tools/verify-live-win.sh`
    ...
    **Card G6 (claim 0487) DONE 2026-08-13 — the draw/window syscall
    ...
    adds slot 15 `sys_win_close` + `win close` → 16) expose the G5
    ...
    `tools/verify-live-win-syscall.sh` PASS 1/1 (`win: windows=3
    focused=2` + `win[2]: user user rect=64,64,256,192` z=2, `syscalls`
    implemented=16 with open=1/fill=4/present=1 + slot 15
    `sys_win_close` registered, and the decoded capture shows the window's
    own content over the terminal).    **Teardown follow-on:** `win close <n>`
