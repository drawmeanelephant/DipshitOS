...
| Forty-three — device depth: USB beyond HID | The "USB-everything" distant mountain: XHCI bulk transfer engine (U1), USB mass storage probe — BOT + minimal SCSI with `[observed]` contract rows (U2), the block-device userland seam + a real consumer (U3), honest device lifecycle (U4), USB serial CDC-ACM probe-first (U5), runner `--usb-msd`/`--usb-serial` flags (U6) | 🔄 **U1+U6 landed 2026-09-07** (PR #1041, claim #1040 — `--usb-msd` runner flag + XHCI bulk engine, `live-usb-bulk` 2/2, issues #1032/#1037 closed); the U1 debugging find (an unrelocated base-0 LLVM pointer table silently killed the guest) became issue **#1042**, whose **loader absolute-relocation pass landed 2026-09-10** (claim #1042, ADR 0019, KRN2 image: 114 `R_AARCH64_ABS64` slots relocated at load, `live-usb-bulk` 2/2 + `live-args` 1/1); **U2 landed 2026-09-10** (claim #1044 — `kernel/src/usb_msc.zig` BOT + minimal SCSI; `usb msc probe` reports INQUIRY `Apple`/`Virtual Disk`, capacity `last_lba=16383`, and a byte-exact 512-byte sector write/read-back; `live-usb-msc` 1/1, `live-usb-bulk` 2/2 zero-regression, host tests green, contract rows added); **U5 recorded negative 2026-09-10** (claim #1046 — Virtualization.framework exposes no USB serial/CDC-ACM device class; the attachable USB classes are keyboard/pointing/mass-storage/host-passthrough, so there is nothing to probe; `[observed]` contract row added); **U3 landed 2026-09-10** (claim #1048 — the file-table `.usb` read-only raw-block volume over `usb_msc` + the EL0 `BLKD.BIN` consumer; `live-usb-block` 1/1 reads the host-staged LBA-1 marker byte-exact); **U4 landed 2026-09-10** (claim #1051 — polled `usb rescan` + administrative `usb detach` with the clean error path; `live-usb-lifecycle` 2/2, zero-regression across `live-usb-bulk`/`live-usb-msc`/`live-usb-block`/`live-usb`). Umbrella #1031, [GH milestone 30](https://github.com/drawmeanelephant/DipshitOS/milestone/30), cards U1–U6 = issues #1032–#1037 — [`docs/march-m43-device-depth.md`](march-m43-device-depth.md) |
