...
- **TABWM polish — issue #1056** (follow-up to #1055) — ✅ **done 2026-09-10**: item 4 test hardening (a `resetForTest()` cold-boot reset every TABWM test calls); item 3 tab navigation + overflow + persistence (Up/Down move the sidebar selection, Enter activates, PageUp/PageDown scroll, a scrolled list window with a `^1-15/16v` indicator, and `.tabs` host-share persistence of order/active re-applied by title as restored windows appear); item 2 window titles for non-tabapp apps (the new seat-gated `WMCTL WINDOW_NAME` cmd 14 resolves the app-set title or the owner process's executable name, so `WINLOOP.BIN` stops showing `App N`); item 1 the true system clock (the boot loader's `EFI RuntimeServices.GetTime` capture rides the **additive** `HandoffV2.boot_time_of_day` field at offset 64 → `timer.wall_time_of_day()` → the new seat-gated `WMCTL CLOCK` cmd 15, so the tray shows real local time with or without a host share). ADR 0004 D5 + the ADR 0015 amendment log updated; class-A `zig build test` 2330/2330.
