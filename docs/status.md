...
| Thirty-four — FAT-free storage (proposed) | Host file channel (HF1–HF7): the guest userland filesystem becomes a macOS folder over the custom-virtio device; FAT32 shrinks to firmware-parsed boot infra | 🔄 proposed (issue #727; `docs/host-file-channel-scoping.md` — flagged by the maintainer to be pulled forward ahead of other pending items) |
