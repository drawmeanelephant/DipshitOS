...
| **1.5 — Interactive Kernel Monitor ("Dipshit Monitor")** | A live, interactive command monitor served by the kernel's serial console (the milestone-two terminal loop becomes its payload) | ✅ **done 2026-08-09** — all 7 hard gates pass; the last (filesystem, claim 3475) closed 2026-08-09; tagged `m1.5-interactive-monitor` |
...
| **Live ESP file-window gate** (fs hard gate) | `bash tools/verify-live-fs.sh` | ✅ **PASS 2026-08-09** | **hard gate 5 closed** — `ls`/`cat`/`write` persist through reboot on real VZ hardware (claim 3475, 1/1 pair): run A persisted `hello world` as `DipshitF:hello.txt` (write-ok) and listed/catted it (`[nvram]`); run B — a fresh boot against the **same** variable store — still listed `hello.txt [nvram]` and printed the content. ESP snapshot (`EFI/`, `KERNEL.BIN`, `BOOTED.TXT`, `MEMMAP.TXT`, `LOADER.TXT`) listed from the pre-exit Simple File System walk; artifacts `live-fs-*`. Root cause fixed en route: VZ's append-per-write store was exhausted after one boot (per-flush TX markers + probe persist ≈ 62 KiB/boot) → now first-flush-only / `-Dprobe-var`-gated |
...
> (claim 3475: `ls`/`cat`/`write` persist through reboot via the pre-exit
> ESP snapshot + NVRAM-persisted writes, `verify-live-fs.sh`, 1/1 pair).
> **Every M1.5 hard gate now passes** (all 7 closed; the last — the
> deferred filesystem one — closed 2026-08-09 by claim 3475); a physical
> allocator and interrupts remain later milestones.
...
- [x] `ls`, `cat`, and `write` persist through reboot — **PASS 2026-08-09 (claim 3475)**: the step-15-sanctioned **pre-exit file window** is implemented and observed end to end on real VZ hardware (`bash tools/verify-live-fs.sh`, class B, 1/1 pair): the kernel snapshots the ESP pre-exit via the EFI Simple File System (`kernel/src/esp.zig`, bounded BSS window) and `write` persists content as the non-volatile runtime variable `DipshitF:<name>`; run A persisted `hello world` and listed/catted it, and run B — a fresh boot against the same variable store — still listed `hello.txt [nvram]` and printed its content (persistence through reboot). The ESP itself stays read-only this milestone; NVRAM variables are the persistence medium, and a full FAT storage driver remains deferred. *(Root cause fixed en route: VZ's append-per-write variable store was exhausted after one boot — the virtio TX flush persisted `M2_TXST!/TXNT!/TXPL!` on every flush (~113/boot) plus the probe persist (~32 KiB) — leaving `write` with `EFI_OUT_OF_RESOURCES`; the per-flush markers/tail are now first-flush-only and the probe persist is `-Dprobe-var`-gated. See claim 3475.)*
...
6. **GIC + timer interrupts (second half of item 5) — ⛔ delivery blocked on VZ (claim 7948).** The GIC (v3: `GICD` @ `0x10000000` / `GICR` @ `0x10010000`, live-probed — VZ's MADT is non-conformant, GTDT PPI 30 standard) and the generic timer (CNTP, 24 MHz) are programmed, read-back verified, and live-gate-tested through an honest **poll-driven heartbeat** (`tools/verify-live-timer.sh`, 3/3). The IRQ-delivery half is **blocked by the platform**: VZ's GIC never presents an interrupt to the guest — GICR is RAZ/WI, PPIs/SGIs/SPIs all inert end to end (full evidence in the claim). Delivering a tick into the claim-9746 IRQ vector reopens on a platform that actually signals; then, and only then, talk about tasks or userspace (deferred).

The command layer above is portable; `docs/march-m15.md` step 15's filesystem-command **deferral is superseded 2026-08-09** by the pre-exit ESP file window (claim 3475 — `ls`/`cat`/`write` persist through reboot; NVRAM variables are the persistence medium, the ESP itself stays read-only). Only a full FAT **storage driver** remains deferred (a later milestone).
