...
| **M61 — Guest self-test** | ADR 0031 landed (#1381); M61b `GOSELF.ELF` + `go-selftest` VZ-green (#1382, PR #1392); M61c intake (#1383) reads the host-seeded `IN/` fixtures and the host byte-compares the guest's copies. #1391 is fixed kernel-side (ADR 0032 — `copy_out` resolves its destination page in the process's own root), the read helper's workaround is deleted and the intake case is its regression test. Next: M61d file-ABI pack (#1384), now unblocked. | #1380 |
