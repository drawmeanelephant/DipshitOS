...
| Forty-three — device depth: USB beyond HID | The "USB-everything" distant mountain: XHCI bulk transfer engine (U1), USB mass storage probe — BOT + minimal SCSI with `[observed]` contract rows (U2), the block-device userland seam + a real consumer (U3), honest device lifecycle (U4), USB serial CDC-ACM probe-first (U5), runner `--usb-msd`/`--usb-serial` flags (U6) | ✅ **done 2026-09-10** — U1+U6 (PR #1041: `--usb-msd` flag + bulk engine, `live-usb-bulk` 2/2; the base-0 pointer-table hazard became #1042/ADR 0019 KRN2); U2 (claim #1044: BOT+SCSI, `live-usb-msc` 1/1, `Apple`/`Virtual Disk`, last_lba=16383, byte-exact write/read-back); U5 recorded negative (claim #1046: no VZ CDC-ACM class); U3 (claim #1048: `.usb` read-only volume + `BLKD.BIN`, `live-usb-block` 1/1); U4 (claim #1051, PR #1052: polled rescan + detach, `live-usb-lifecycle` 2/2); closeout (claim #1053: fleet 7/7 at HEAD, umbrella #1031 + [GH milestone 30](https://github.com/drawmeanelephant/DipshitOS/milestone/30) closed 6/6). Detail: [`docs/march-m43-device-depth.md`](march-m43-device-depth.md) |
...
- **Self-hosting seed — in-guest compiler + window depth** (issue #620) — [GH milestone 20](https://github.com/drawmeanelephant/DipshitOS/milestone/20) — 5 open / 23 total as of 2026-09-03: the Z0→Z4b compiler ladder is fully closed (#749/#756/#757/#758/#759/#760/#761); remaining cards are the Lane 0 hygiene trio (#773/#775/#776) and the two umbrellas (#706/#707). **Lane 1 window depth (umbrella #707) is 3/4 cards landed as of 2026-09-04**: WM1 — window ceiling 4→8 with page-pool back-buffers (PR #922); WM3 — taskbar depth (PR #974, claim #972): current-workspace id-ascending entries via the shared wnd_core entry rule (kernel render + WM hit-test drift guard), titles clipped to the 8-char entry slice, focused active-fill + 2px underline, minimized darker fill + restore dot, WM-driven click-to-focus/restore via new slot-65 TASKBAR (cmd 12) + `wm:` taskbar= counter + `dui taskbar` enumeration report, and the WMS5 mirror fix (minimize now fans). Class-B gate `verify-live-wm3-taskbar.sh` PASS 3/3 boots on VZ (shim enumeration zero-regression + WM focus click + WM restore click). WM4 — paint depth (PR #979, claim #976): WM-driven rest opacity via the backward-compatible ChromeDesc v2 (rest_alpha tail, len 44; v1 40-byte submitters zero-extend to the identical opaque default), applied only to the client blit of an at-rest unfocused window (chrome stays opaque — WMS4 pixel parity re-verified untouched) + snap-preview corner brackets; class-B gate `verify-live-wm4-paint.sh` PASS 2/2 boots on VZ with exact blend-math pixel probes. Rounded corners deferred (needs a chrome-geometry ABI v3, would break WMS4 pixel parity). Remaining: WM2 mission-control — ported on `agent/oxalpha/wm2-overview` (claim #980, class-A green), class-B gate blocked on the class-B harness flake (#990). The one loose end the Z4b capstone exposed — the z2a zc-leg OUT.TXT flake (zc-compiled binary intermittently lost its file-channel write while the host-built twin was deterministic) — was **root-caused and FIXED 2026-09-03 (claim #899)**: an SMP data race in the guest file channel. The virtio_file request ASSEMBLY into the module-global `vf_req_buf`/`vf_write_buf` ran outside the spinlock (only the submit/wait/free was locked), so two cores (the app on the secondary, the shell's history save / exec reads on the primary) spliced encodes — the host's `request length mismatch` refusals — and the app's silently-ignored failures left OUT.TXT empty/missing while it still exited 72. Fix: the `IrqSaveSpinlock` now covers encode+exchange in `exchange()`/`write()`; timed-out chains are PARKED (never recycled until their reply is observed — `virtio_custom.park_chain`/`reap_parked`); timeouts re-kick and retry once; the wait budget grew 32M→320M (~155 ms → ~1.5-2 s measured). Verified: z2a zc leg 12/12 consecutive PASS (0 length mismatches, 7 boots ran the app on the secondary core — the previously risky condition; pre-fix rate 2/7), full corpus sweep 12/12 cases × both legs, `verify-live-zc.sh` PASS. Evidence: `artifacts/claim-899/`. **VL6's open run-parity end is now closed too (claim #988, 2026-09-04):** the first live RUN consumer of the GUI surface landed as an in-guest-compiled Snake — a new 4-file corpus case `snk` (dual-run in `verify-zc-corpus.sh`, both legs PASS, exit 72, ordered markers snake-up → snake-wait → snake-move → snake-over) and a display-backed class-B gate `verify-live-snake.sh` that compiles the group in-guest, runs it windowed, and proves the game's pixels (dark board + green head + pink food) in framebuffer captures. It also exercises the raw `zc.svc(21, &buf)` event-poll seam (ADR 0009) from zc code for the first time. Host-shim `zc.svc` widened to anytype (`user/src/lib/zc.zig`) so `zc.svc(21, &buf)` type-checks under host Zig. See the gate table row + `docs/line-of-sight.md`.- Numeric-milestone pointers for M32/M33 row above (milestones 16/17); the full**The in-guest single-read source cap was lifted 2026-09-04 (claim #992):** zc now drains each source with chunked file_reads into a shared 32 KiB source arena (`user/src/zc.zig` `source_arena_cap`, +8 KiB bss over the old per-file staging), so a source (or compile group) up to the arena compiles in-guest — a game can live in one >2048-B file. The old behavior silently truncated anything above the kernel's 2048-B single-read clamp. New corpus case `big` (the snake as one 4744-B source, dual-run, markers past byte 2048) PASS on both legs; regression sweep z05/z2a/z3a/z3b × both legs PASS (sha256 file pins byte-exact).
- Numeric-milestone pointers for M32/M33 row above (milestones 16/17); the full  per-card M33 detail lives in [`docs/march-m33-seam-b-pixel-ownership.md`](march-m33-seam-b-pixel-ownership.md).
...
- **M40 — Gate Fleet Consolidation** ([GH milestone 27](https://github.com/drawmeanelephant/DipshitOS/milestone/27)) — **✅ done 2026-09-06, 6/6** (GF1–GF6, issues #934/#937/#938/#939/#940/#931): 203 hand-edited `tools/*.sh` gate scripts (38,036 lines of `verify-*.sh`) consolidated into 181 declarative vgate specs run by the single harness `tools/gate/vgate.sh` (GF2), discovered by `tools/gate/fleet.sh` (GF5) — the spec dir is the single source of truth feeding just, CI sharding, and the generated [`gate-fleet-inventory.md`](gate-fleet-inventory.md) (GF1, `--check` enforced in CI). GF6 closeout full run: class-A portable set green + class-B fleet 209 boots on the reference host (10,052 s serial, ~54 s/boot); scoreboard — scripts 203→31, verify-*.sh lines 38,036→0 (896 lines of shared gate infra), just-recipe coverage 40/203 (~20%)→3 recipes covering all 185 fleet members, inventory coverage ~40%→100% generated. New-gate rule (specs only, no one-off `verify-*.sh`) recorded in AGENTS.md + [`testing.md`](testing.md). Two latent flakes were surfaced and fixed in #1020 (two specs missing the SPIKE runner declaration; the `live-editor` `--screen ''` PNG leak) and the pre-existing `live-wasm` red was fixed 2026-09-06 on #1020 — root cause was not the window model: the fixture's 15M-iteration hold was calibrated "~10-15s" and the interpreter since outgrew it (hold → milliseconds, window closed before script2), while the 4-deep exit-report rings dropped floatapp's exit-590 when five wasm execs began exiting in one drain window. Fixes: poll-hold fixture (window lives until the gate's `dui close`), both exit-report rings 4→8, spec choreography pins the live dui row + close.
- **Repo tooling — the class-A registration guard now actually runs in CI** (issue #1186, 2026-09-12): `just verify-portable` owns the portable set, but the macos CI job hand-copied a subset of it and three class-A gates had silently gone local-only — `bash tools/inventory-gates.sh --check` (the GF1 registration guard, which this file and [`docs/testing.md`](testing.md) had been *claiming* CI enforced since 2026-09-06), `tools/verify-ttf-fonts.sh`, and `tools/verify-vf-class-a.sh`. All three are now steps in `.github/workflows/ci.yml` (found by the new check, not by hand), and `tools/lint-workflows.sh` asserts that every command in the recipe is present in `ci.yml`, so a portable gate cannot go local-only again without failing CI. The same pass audited `tools/` for the locale-sensitivity class behind issue #1177: no other tool renders a tracked file, and the corpus gate's locale-dependent case-list dedupe and case→artifact-name mapping are pinned (see [`docs/testing.md`](testing.md), "Locale determinism").
...
- **M42 — The Sexiburger Desktop** ([GH milestone 29](https://github.com/drawmeanelephant/DipshitOS/milestone/29); detail tracker: [`docs/march-m42-sexiburger-desktop.md`](march-m42-sexiburger-desktop.md), umbrella #981) — 🔄 **tranches 1+2 landed 2026-09-04** (claims #987/#996): SX1 the proper 🐙+🍔 mascot raster (the real artwork `assets/sexiburger.png`; the old `assets/mascot.png` was unrelated art) in the TABWM sidebar, God Menu header, and SEXIBURG.BIN via the new `ui.draw_image_buf` scanout blit; SX2 the WM→app resize seam (`wm_apply_rect` emits `WIN_RESIZE` on size change) + TABWM's idempotent full-viewport proposal behind the additive `declare_fullscreen` RPC (kind 8; WND refuses — zero regression); SX3 `lib/tabapp.zig` (open+declare+dispatch+scale, either-seat WM discovery) with CALC ported as the full-screen exemplar; SX5 the TABWM Sexiburger overlay (APPS.TXT palette, launch into a new tab) + the settings-driven default-manager seam (`settings set wm tabwm`; the default VM stays shim-only) + the capstone Class-B gate `verify-live-tabwm-fullscreen.sh` **PASS 2/2** (settings-seeded autostart + full-viewport CALC relayout; overlay launch into a tab). Live regressions: `verify-live-tabwm.sh` PASS, `verify-live-godmenu-summon.sh` PASS, `verify-live-sexiburger-actions.sh` PASS 2/2 (one #843-shaped flake). calc-depth/calc-prog/tabstrip fail identically on baseline main @2762e68 on this host (pre-existing, recorded in the march doc). Remaining: the #985 ≥3-app live gate (fleet code-landed — all eight apps tab-aware, class A green: per-app host suites + `zig build` + `zig build test` 126/126) + the documented human-session default flip. **UX hardening round 2 landed 2026-09-05** (claim #1011, ADR 0018 addendum — see [`docs/march-m42-sexiburger-desktop.md`](march-m42-sexiburger-desktop.md) "UX hardening round 2"): TABWM unsaved-state honesty (mirror bit 12 → dirty dot + a modal unsaved-changes dialog on every close entry point, riding the existing slot-65 DIALOG actions 3–6 with wnd_core button parity; TABWM self-paints the modal because its full-scanout compose overdraws the kernel's blit), close-feedback flash, and Alt-Tab parity (shared `alt_tab_next` policy, kernel ALT_TAB commit) — no new kernel surface; class A 75/75, both new class-B gates **PASS on live VZ** (unsaved 2/2, alttab 1/1 — see the gate table row below and `artifacts/2026-09-05-tabwm-unsaved-alttab/`).
- **Terminal (vt) seam — issue #1072 (M44), done 2026-09-11** (claims #1073/#1075): ADR 0020 fixes the ABI — a terminal is a **device file** (`/dev/tty` via the existing `sys_file_open`/read/write slots; terminal I/O adds **no** syscall) backed by a pure, host-tested `kernel/src/terminal.zig` (output ring + input queue + exclusive front-end attach). `kernel/src/file_table.zig` routes `/dev/tty` to a process's controlling terminal; the serial front-end pump (console RX → input queue, output ring → console TX) is driven from the syscall path; the only new slot is **ADR 0007 slot 67 `sys_tty_attach`** (`implemented_count` 68). Boot default unchanged — the kernel monitor keeps the raw console until a process attaches. Proven end to end by the class-B pilot `TTYECHO.BIN` (`live-ttyecho`: EL0 opens `/dev/tty`, attaches, echoes a scripted line into the serial log) and by class-A pump/syscall tests; `just verify-portable` green. Next: the window (`TERM.BIN`) and net (remote/SSH) front-ends, then the userland shell on top.
- **M45 — Userland shell & terminal front-ends** (umbrella #1067, card tracker [`march-m45-userland-shell.md`](march-m45-userland-shell.md)) — 🔄 **SH1 landed 2026-09-11** (claim #1086, issue #1077): the shared userland terminal library `user/src/lib/tty.zig` (ADR 0021 D6) — open `/dev/tty`, attach a front-end (`sys_tty_attach`, slot 67), raw-ish byte read/write through the seam, and a pure line editor (insert/delete, Left/Right, Home/End, Ctrl chords, Up/Down history) with a bounded 16-entry history ring. The editing surface is host-tested (15 class-A tests: key decoding, insert/delete/Home/End, history wrap + dedupe + draft, line submission, overflow refusal) and the syscall glue is thin. Live proof: `TTYED.BIN` drives the editor over the serial console (`live-ttyed`, class-B, **PASS 1/1**) — a scripted burst types `helo`, Left + insert `l` → `hello`, recalls `hello` via Up, then Home/End-inserts `zxy!`, each submitted line echoing `ttyed: line <x>`. No kernel changes; `just verify-portable` green. **SH2 landed 2026-09-11** (claim #1088, issue #1078): `SH.BIN` (`user/src/sh.zig`) is the userland shell process on the seam — open `/dev/tty`, attach the serial front-end, prompt, read lines through the SH1 editor, and dispatch via the pure, host-tested core `user/src/lib/shell.zig` (tokenizer, `$VAR`/`${VAR}`/`$?` expansion, bounded env + alias tables, cwd normalization, builtin classification, PATH-like candidate resolution, and a line interpreter returning `print`/`run`/`source`/`exit` actions). Builtins: `echo pwd cd exit env set unset export printenv alias unalias history prompt type which true false help source jobs fg`; externals resolve `foo` → `foo.BIN`/`foo.ELF` → `FOO.BIN`/`FOO.ELF` and run foreground through the additive `sys_wait` (slot 8) wrapper, propagating `$?`. Class-A: 18 new shell-core tests + `zig build test` / `just verify-portable` green. Live proof `live-sh` (class B, **PASS 1/1**): type `echo sh-echo-ok`, `cd /data` + `echo PWD=$PWD` → `PWD=/data`, run `status43` (case-insensitive resolution → `STATUS43.BIN`), then Up-history re-runs it (`status43: alive` twice). No kernel changes. **SH3 landed 2026-09-11** (claim #1090, issue #1079): Tab completion with candidate cycling and Ctrl+R reverse-i-search, ported from the M19 kernel editor. `user/src/lib/tty.zig` gains an injected `CompleterFn`/`CompletionMatch` (single-match suffix + trailing space, repeated-Tab candidate cycling, mid-line tail preservation) and an in-editor reverse-i-search over the history ring (query edit, live match load, Enter-accept / Esc-cancel restoring the draft). `user/src/lib/shell.zig` gains a pure sorted/de-duplicated candidate set (builtins + aliases + share files with `.BIN`/`.ELF`/`.SO` basenames, case-insensitive prefix); `user/src/sh.zig` wires the completer. Class-A: new tty completion/search tests + a `complete()` test (`zig build test` / `just verify-portable` green). Live proof `live-sh-complete` (class B, **PASS 1/1**): `hel`+Tab → `help` → `builtins:`, then Ctrl+R `status` recalls the `status43` history entry and re-runs it (`status43: alive` twice, `reverse-i-search` in the log). No kernel changes. **SH4 landed 2026-09-11** (claim #1092, issue #1080): operators. New pure `user/src/lib/pipe.zig` ports the M19 `pipe_split` (first `|` outside quotes; >1 refused), `redirect_split` (`>`/`>>`/`<`), and an iterative fnmatch-style `glob_match` (`*`/`?`/`[a-z]`); `user/src/lib/shell.zig` gains tokenizer wildcard flags, glob expansion against the share listing (byte-sorted, nullglob-off), and a `cat` builtin that emits bound stdin; `user/src/sh.zig` orchestrates a pipeline through the kernel pipe slots 56/57 (`sys_pipe_write` left output → `sys_pipe_read` right stdin), `>`/`>>` capture builtin output to a share file (MODE_CREATE / MODE_APPEND), and `<` reads a file into the command's stdin. Honest bound: pipes/redirection capture builtin output — external apps write fd 1 directly. Class-A: pipe/redirect/glob matcher tests + shell glob-expansion and `cat` tests; `zig build test` / `just verify-portable` green. Live proof `live-sh4` (class B, **PASS 1/1**): `cd /data` + `env | cat` → `PWD=/data`, and `help > HELP.TXT` + `cat < HELP.TXT` → `builtins:`. No kernel changes. **SH5 landed 2026-09-11** (claim #1094, issue #1081): scripting, ported from M19. New pure `user/src/lib/script.zig`: `chain_split` (`;`/`&&`/`||`), a recursive-descent `$(( ))` arithmetic evaluator/expander, `$(...)` location scan (arithmetic-aware), whole-word `parseIf`/`parseFor`/`parseWhile`, `;`-body splitting, and a bounded `FuncTable` (`fn NAME(args) { ... }` define/find/redefine). `user/src/lib/shell.zig` integrates arithmetic expansion in `execute`, a `FuncTable` with `$0`/`$1..$N`/named-arg binding and a `call` action, and `break`/`continue` loop signals. `user/src/sh.zig` gains the `runLine` orchestrator: `fn` defs, `$()` substitution via capture, single-line `if`/`for`/`while`, chains with short-circuit, then the SH4 per-segment dispatch; `source` runs each file line through `runLine`. Class-A: 9 script-core tests + function/arith/loop-signal shell tests; `zig build test` / `just verify-portable` green. Live proof `live-sh5` (class B, **PASS 1/1**): a `SCRIPT.SH` on the share exercises `set X=$(( (2+3)*4 ))` → `ARITH=20`, `for n in a b c`, `if true; then …`, `fn greet(name)`, `echo SUB=$(echo INNER)`, and `true &&` / `false ||` short-circuit (with `IF-NO`/`CHAIN-NOPE` proven absent). No kernel changes. **SH6 landed 2026-09-11** (claim #1098, issue #1082): `TERM.BIN` — the shell in a TABWM window (ADR 0020 Amendment A, A1–A6). Kernel: `kernel/src/terminal.zig` gains a window binding + a bounded per-terminal `Screen` grid + scrollback and the pump entry points (`pumpWindowOutput`/`windowTerminal`/`detachWindow`/`screenOf`); `kernel/src/syscall.zig` `handle_tty_attach` implements selector `2` (`window_id` — caller must own the `.user` window, exclusive with serial, `0` detaches, `3` stays `ENOSYS`); `kernel/src/file_table.zig` drains a window-bound `.tty` write into the grid and marks the bound window damaged; `kernel/src/driving_award.zig` renders the grid into the bound window with the kernel glyph raster (client area below the title bar) and auto-detaches on close/owner-exit; `kernel/src/input.zig` encodes focused-window keys with `hid_to_bytes` into the bound terminal's input queue (A5) without stealing focus. User: `user/src/term.zig` opens a `.user` window, opens `/dev/tty`, attaches `(2, id)`, and runs the shared shell core (`lib/shell.zig` + `lib/tty.zig`) over the fd — draws no pixels itself. Class-A: new `terminal.zig` screen/binding/pump tests, an `input.zig` A5 routing test, and the extended `SYS_TTY_ATTACH` syscall test; `zig build test` / `just verify-portable` green. Live proof `live-term` (class B, **PASS 1/1**): exec `TERM.BIN`, type `echo hi` over the WM input seam, observe `term: ready`/`term: attached`/`term: line echo hi`/`term: done status=0` and the `dui` registry entry `user user rect=64,48,640,400`, and a decoded snapshot shows the rendered shell glyphs in the window client area. Boot default unchanged — `live-ttyecho`, `live-ttyed`, `live-sh`, `live-win`, `live-win-close`, and `live-desktop-typing` all stay **PASS**. **SH7 design landed 2026-09-11** (claim #1100, issue #1083): [ADR 0020 Amendment B](decisions/0020-terminal-seam.md) fixes the net/remote TCP front-end (selector `3`) — kernel-pumped and owner-hosted (the terminal's process enters LISTEN via `sys_tty_attach(3, port)`, no cross-process capability), raw bytes, **plaintext trusted-network-only** posture, disconnect/owner-exit auto-detach with the serial console unreclaimed, and a host→guest inbound class-B gate via a new runner TCP-client seam (`--net-tcp-connect`). **SH7 landed 2026-09-11** (claim #1102, issue #1083): implemented per ADR 0020 Amendment B. Kernel: `kernel/src/terminal.zig` gains the net front-end binding (`attachNet`/`attachedNet`) and the pump (`pumpNetInput`/`pumpNetOutput`) over the single bounded TCP connection, auto-detaching on a peer FIN/RST (emitting the `tty net: detached` serial marker); `kernel/src/syscall.zig` `handle_tty_attach` implements selector `3` (`port` — caller owns the listener, exclusive with serial/window, `0` detaches); `kernel/src/file_table.zig` drives the net pump on `.tty` read/write; `kernel/src/tcp.zig` records a clean peer FIN (`peer_fin`). User: `SH.BIN net [port]` (default serial unchanged) attaches the net front-end and runs the shared shell core; `lib/tty.zig`/`abi.zig` add `attachNet`/`tty_attach_net`. Host: the runner gains `--net-tcp-connect <guest-ip>:<port>[:<payload>]` (+`-after`/`-close-after`) — a deterministic host-initiated inbound TCP client (SYN → handshake against the guest's fixed server ISN → payload → reply → FIN). Class-A: new `terminal.zig` net-binding test, `tcp.zig` peer-FIN reset, the extended `SYS_TTY_ATTACH` syscall test; `zig build test` / `just verify-portable` green. Live proof `live-remote` (class B, **PASS 1/1**): `exec SH.BIN net 2323`, the host connects inbound, sends `help`, captures `builtins:` from the shell's reply, then FINs and the terminal auto-detaches. Boot default unchanged — `live-sh`, `live-sh-complete`, `live-sh4`, `live-sh5`, `live-httpd`, `live-net-tcp`, `live-term`, and `live-ttyecho` all stay **PASS**. **SH8 landed 2026-09-11** (claim #1106, issue #1084) — **M45 complete**: the default-shell flip (ADR 0021 D5). `settings shell=sh|monitor` (new `settings.zig` key, default `monitor`); at boot the kernel login seam (`kernel/src/shell.zig` `login_handoff`) execs `SH.BIN` and relinquishes the raw console (the monitor stops reading it; the serial terminal front-end owns the RX, and the monitor resumes if the shell detaches). `user/src/sh.zig` runs a `STARTUP.SH` startup file and adopts the persisted `prompt` key; the default `monitor` boot is unchanged. Class-A: new settings login-shell test; `zig build test` / `just verify-portable` green. Live proof `live-shell-default` (class B, **PASS 1/1**): boot 1 (no SETTINGS.TXT) stays in the monitor and persists `shell=sh` + a prompt, `reboot`, boot 2 prints `login: shell=sh -> SH.BIN` and lands in `SH.BIN` (`sh: ready`/`sh: attached`, typed `sh8-ok`, adopted prompt). Boot default unchanged — `live-settings`, `live-args`, `live-sh`, `live-sh5`, `live-reboot`, `live-term`, `live-remote`, `live-win`, `live-ttyecho` all stay **PASS**.
- **M46 — Remote access Stage 0/1** (umbrella #1108, goal #1066, [ADR 0022](decisions/0022-remote-console.md)) — ✅ **done 2026-09-11**, RC0–RC4 (issues #1109/#1069/#1111/#1104/#1105/#1112). **RC0 design** (ADR 0022): the host bridge transport (`--console-tcp [host:]port[:secret]`, raw, single-client, host-only — RC1 landed via PR #1070), the v1 **guest** auth primitive (an in-band shared secret as the session's first line, checked by the kernel net pump — the shell never sees it; optional opt-in source-IP allowlist), the explicit **plaintext / trusted-network** posture, and the declarative during-run **vgate TCP client hook** (#1069). **RC2** (#1069): `tools/lib/vgate-client.py` + the `vgate_client`/`client-contains` spec-format extension (SPEC.md); pilot `live-console-tcp` (class-B **PASS 2/2**: hook drives `--console-tcp`'s monitor, negative `--expect-fail`). **RC3** (#1111): `sys_tty_attach(3, port, secret_ptr, secret_len, allow_ip)`, the terminal net-auth state + pump gate (pre-auth bytes are never delivered; a mismatch sends `auth failed` and detaches), the source-IP allowlist RST at the TCP layer, `SH.BIN net <port> [secret] [allow-ip]`, `TERM.BIN net` (#1104) via the shared `lib/netargs.zig`, and the half-open accept timeout + comptime `NetSeam` injectable pump (#1105, class-A host-tested). Class-B `live-remote-auth` **PASS 2/2** (wrong secret rejected, right secret drives the shell) and `live-term-net` **PASS 1/1** (#1104: `TERM.BIN net 2323` hosts the front-end — net mode opens no window). **RC4** (#1112): `live-remote-console` **PASS 2/2** (the client hook proves the bridge secret reject/accept and drives the guest shell). **Boot default unchanged** (must-observe): `live-remote`, `live-sh`, `live-sh5`, `live-term`, `live-httpd`, `live-net-tcp`, `live-ttyecho` all **PASS**.
- **M50 — Trust & isolation baseline** (umbrella #1133; design card TS0 #1134 → [ADR 0024](decisions/0024-trust-and-isolation.md) + [`docs/trust-scoping.md`](trust-scoping.md); cards TS1 #1135, TS2 #1136, TS5 #1139, TS4 #1138, TS3 #1137) — ✅ **done 2026-09-11 — GH milestone 37 6/6 cards closed** (TS0 #1134, TS1 #1135, TS2 #1136, TS5 #1139, TS4 #1138, TS3 #1137; umbrella #1133 closed). **TS1 identity landed 2026-09-11** (claim #1153); **TS1 identity:** `Process` gains `uid`/`caps` (ADR 0024 D1/D2 — `uid_system=0`, `uid_user=1000`, no caps), assigned at `process.create` (`create_as` for an explicit kernel principal), preserved by `exec` (no setuid; the EL0 `sys_exec` path passes the caller's principal), and never settable by a syscall (there is no setter). New **ADR 0007 slot 68 `sys_principal(buf)`** returns the caller's `{u32 uid, u32 caps}` read-only (8 bytes LE; `EINVAL` non-process, `EFAULT` bad buffer); `implemented_count` 68→**69**; the 40-byte `sys_procs` snapshot row stays byte-frozen; the monitor `procs` report gains `uid=`/`caps=` columns. Userland: `whoami`/`id` builtins (pure-shell `PrincipalView`, glue reads slot 68). Class-A: process/exec/syscall/shell host tests + `zig build test` **green**; class-B `live-trust-whoami` **PASS 1/1** on VZ (monitor `procs` row `uid=1000 caps=0` agrees with `whoami`/`id`); boot-default unchanged — `live-sh`, `live-shell-default`, `live-help`, `live-remote-auth` all **PASS**; `just verify-portable` green, `inventory-gates --check` green. **TS2 permissions landed 2026-09-11** (claim #1155). New `kernel/src/trust.zig`: a bounded 64-entry BSS metadata table keyed by the `parse_path`-normalized `(Partition, path)` pair, the single pure predicate `check(actor, partition, path, want) -> allow|eacces|enoent` (owner bits when `caller.uid == entry.uid`, else other bits; `CAP_FS_ANY` bypass for kernel actors), and `OWNERS.TXT` `#v1` load/save (`path<TAB>mode<TAB>uid<TAB>flags`; malformed or unknown-schema lines deny their path, fail closed; table-full ⇒ `ENOSPC`, never eviction). Default policy: file 0644, dir 0755, owner `uid_user`, `secret=false`; `.usb` read-only 0444; `.tty` device semantics; the group triplet is reserved and normalized to zero. Keys compare case-insensitively with case-preserving storage — the ADR 0024 D4 case-insensitive-host-share risk resolved. `trust.check` is wired into every `file_table` entry point (`open`/`read`/`write`/`delete`/`rename`/`truncate`/`dir_list`) after `parse_path` and before any `virtio_file` access, and every content read/write consumer declares an explicit `kernel_actor()` (monitor `cmd_vf`/`cat`/`stat`/`write`/`mktemp`/`sh <script>`, `exec`, `settings`, kernel-shell history/env/`.virelairc`/`WINDOWS.SAV`, `tombstone`, `redirect`; directory-name listings are intentionally exempt — D8 allows secret names, not content). The secret class (`SECRETS.TXT`, `secret=true`) is denied read/list/delete/create through the file ABI for EVERY actor, including the monitor's `vf cat`. New **ADR 0007 slot 69 `sys_file_mode(path, mode)`** — owner-only chmod on an existing path or `CAP_FS_ANY`, no chown, persists to `OWNERS.TXT`; `implemented_count` 69→**70**; EL0 `chmod` builtin (`ModeView`, `sys_file_mode`) + the shell's file-error lines now render the ADR 0007 mnemonic (`EACCES`). Class-A: 12 new `trust.zig` tests (allow/deny matrix, default policy, secret class, malformed fail-closed, table-full ENOSPC, `OWNERS.TXT` round trip, case key, remove/rename) + the extended syscall/monitor/shell suites; `zig build test` **green**. Class-B `live-trust-modes` **PASS 1/1** on VZ (host-seeded `OWNERS.TXT`: uid_user `cat < TARGET.TXT` → `EACCES`; an unlisted file follows the default policy and reads; the owner `chmod` persists an `OWNERS.TXT` the host inspects; `vf cat SECRETS.TXT` is denied at the direct-consumer seam; the secret value never appears in the transcript). Boot-default unchanged — `live-trust-whoami`, `live-settings`, `live-history`, `live-user-fs`, `live-sh4`, `live-fs`, and `live-vf` (6/6) all **PASS**. **TS5 secrets landed 2026-09-11** (claim #1157). New `kernel/src/secret.zig`: the `SECRETS.TXT` secret-class store — the settings engine's bounded key/value semantics (`max_key_len` 32, `max_val_len` 64, `#v1` header; 64 chars hold a 32-byte key or Ed25519 seed in hex exactly) with `max_secret_entries` 8 and a DISTINCT file; one `key<TAB>uid<TAB>value` line per entry scopes each secret to a principal. The class holds **by construction**: `trust.ensure_secret_file()` registers `SECRETS.TXT` (`secret=true`, `0600`, `uid_system`) before a byte is read, and refuses to load if the bounded metadata table cannot hold it (fail closed). New **ADR 0007 slot 70 `sys_secret_get(buf, len)`** — the ONLY in-guest reader, returning the CALLING principal's entries as fixed `SecretRecord`s (108 bytes) through uaccess; `EINVAL` non-process/too-small buffer, `EFAULT` bad buffer; **no `sys_secret_set`** (host-side provisioning; deferred per D8). `implemented_count` 70→**71**; slot 71 (`sys_tty_net_auth`) reserved for TS4. Monitor `secrets` command + EL0 `secrets` builtin print key **NAMES only**, never values, never mutate. **Never-logged contract** (the acceptance heart): `sys_secret_get` and slot 71 are excluded from strace argument/return tracing; class-A redaction tests assert a known value is absent from strace, monitor `secrets`/`procs` output, the `sys_procs` snapshot, and crash tombstones while its NAME is present. Class-A: `secret.zig` bounds (8 entries, 32/64 caps)/round-trip/per-uid tests, `trust.ensure_secret_file` by-construction tests, the syscall owner-only + EFAULT/EINVAL contract, and the strace/snapshot/tombstone redaction tests; `zig build test` **green**. Class-B `live-secrets` **PASS 1/1** on VZ (a host-seeded value is listed by NAME in both the monitor `secrets` and the EL0 `secrets` builtin; `cat < SECRETS.TXT` and `vf cat SECRETS.TXT` are denied at both seams; the VALUE is `serial-absent` while its NAME appears; the host share is unmodified). Boot-default unchanged — `live-trust-whoami`, `live-trust-modes`, `live-settings`, `live-help`, `live-sh`, `live-sh5`, `live-vf` all **PASS**; `just verify-portable` green, `inventory-gates --check` green. **TS4 authenticated remote landed 2026-09-11** (claim #1159, issue #1138). The M46 in-band shared secret is deleted (`net_secret`/`secretEq` gone): on TCP accept the kernel net pump mints a fresh 32-byte challenge from `csprng` and frames one line `VIRELAIOS-AUTH/1 <scheme> <hex-challenge>\n`, buffers one length/hex-validated reply line, and gates every post-challenge byte on the attached process's verdict. Reject, malformed, or no verdict within the 10 s deadline is `auth failed` + `tcp.reset()` + detach; pre-auth bytes never reach the terminal input queue. New **ADR 0007 slot 71 `sys_tty_net_auth(op, buf, len)`** (op 0/1/2 = challenge out / reply out / verdict in; caller must own the attached net terminal; strace-excluded with slot 70); `implemented_count` 71→**72**. `sys_tty_attach` selector 3's secret arguments are replaced by a scheme (0 open, 1 hmac-sha256, 2 ed25519) — the credential never rides argv. Bounds: `tcp.payload_max` 64→**192**, `segment_max` 84→**212** (`frame_max` 246), `net_auth_line_max` **160**; every staging consumer audited (TCP send/recv staging auto-grow; the 193-byte RX-overflow test pins the new bound). New `user/src/lib/netauth.zig`: reads the challenge/reply through slot 71, verifies `hmacSha256` + `ct.ctEq` over the domain-separated message (or `ed25519.verify` over the `"VIRELAIOS-AUTH/1 ed25519"` message), votes, and zeroizes key material; keys come from the TS5 store (`net-hmac` byte-for-byte, `net-ed25519` 64-hex public key). CLI `net <port> [open]`: credential ⇒ auth mandatory; no credential ⇒ refuse to listen unless `open` is explicit (fail closed). Stage-0 `--console-tcp [host:]port[:secret]` is now HMAC-SHA256 challenge-response via CryptoKit, `vgate-client.py --hmac-secret` answers it with Python's stdlib `hmac`; no secret ⇒ byte-identical to before. Class-A: the pump state machine over the injectable `NetSeam` (fresh challenge, malformed/wrong reply, stale/rebound challenge, deadline, wipe, framing bounds), pinned HMAC + CryptoKit-signed Ed25519 vectors, the slot-71 owner/strace contract, and the updated table/report tests; `zig build test` **green**. Class-B `live-remote-auth2` (wrong MAC → `auth failed`; right MAC drives the shell; a captured MAC replayed against the fresh challenge is rejected; explicit `open` accepts); `live-remote` and `live-term-net` re-pointed to `open`, `live-remote-console` updated to `--hmac-secret`, `live-remote-auth` retired/removed. Boot default unchanged: nothing listens unless a process asks. **TS3 privilege gating landed 2026-09-11** (claim #1161, issue #1137) — the milestone’s last card. No new syscall slot: `implemented_count` stays **72**. New explicit, bounded `capability_gates` table in `kernel/src/syscall.zig` (one row: `sys_kill` → `CAP_PROC_ADMIN`, queried via `gated(number) ?cap`) so a future gate is one row plus its handler’s explicit query. `sys_kill` (29) allows self and same-uid targets for every principal and returns **EACCES** for a cross-principal kill unless the caller’s `caps` holds the gated capability; the principal check runs BEFORE the target-state checks (no foreign-state leak); the EL1h monitor is never a target (it has no pid; `scheduler.request_kill` independently refuses the shell/idle slots); `sys_exec` (28) stays ungated and principal-preserving (TS1), and a class-A audit walks every implemented slot name + the gate table to prove there is no elevation syscall. The EL1h monitor’s `exec` gains the admin-spawn flag **`exec [-c<core>] [-u<uid>] [<file> [arg...]]`** — `-u0` assigns `uid_system` + `kernel_caps`, `-u1000` the default `uid_user`/no caps, any other uid refused with a decimal-overflow guard; it is the only path that names a principal, and EL0 has none. Class-A: the gate-table shape + the ungated D10 rows, the no-elevation audit, cross-principal EACCES / same-uid + self allowed, and the privileged `uid_system`+`CAP_PROC_ADMIN` cross-principal kill (4 new syscall tests) plus the `exec -u` CLI vocabulary tests; `zig build test` **green**. Class-B `live-trust-caps` **PASS 2/2** on VZ: a monitor admin-spawned `uid_system` COUNTER.BIN (`procs: id=1 name=COUNTER.BIN uid=0 caps=3`) is untouchable from a `uid_user` TOP.BIN — `top: kill pid=1 err=-7`, `29 sys_kill calls=1`, and the probe’s `counter: alive` markers continue (nothing armed) — while the boot-default all-`uid_user` control still kills same-uid (`top: kill pid=1`, `tasks user-exec exited status=137`, markers stop). Boot default unchanged — `live-trust-whoami`, `live-trust-modes`, `live-secrets`, `live-kill`, `live-sys-kill`, `live-procs` all **PASS** (the TS3 fleet run surfaced two stale `procs`-row regexes in `live-kill`/`live-sys-kill`, which still expected the pre-TS1 row shape; the specs now match the documented `uid=`/`caps=` columns and both PASS). `just verify-portable` green, `inventory-gates --check` green. This closes umbrella #1133 and GH milestone 37.
- **M51 — SSH (goal #1066 Stage 3; umbrella #1164; design SSH0 #1165 → [ADR 0025](decisions/0025-ssh.md) + [`docs/ssh-scoping.md`](ssh-scoping.md))** — ✅ **done 2026-09-12, all eight cards closed** (SSH-P1 #1166, SSH-P2 #1167, SSH1 #1168, SSH2 #1169, SSH3 #1170, SSH4 #1171, SSH5 #1172). The guest gained a real SSH-2 **client**, `SSH.BIN`: one modern profile (`curve25519-sha256`, `ssh-ed25519`, `chacha20-poly1305@openssh.com`, `publickey`), built entirely in userland on the M47 primitives over the SSH1 stream/packet adapter — the kernel gained **exactly one** slot, **ADR 0007 slot 72 `sys_getrandom`** (`implemented_count` 72→**73**), and stays crypto-free. SSH5 (the last card) landed the class-B endpoint proof and the deterministic host peer: `VMRunner` gains `--net-tcp-respond <ip>:<port>:ssh` — a runner-hosted minimal SSH-2 server in the new Virtualization-free `VSSH` Swift library (CryptoKit X25519/Ed25519 + a **hand-rolled** djb ChaCha20/Poly1305 OpenSSH cipher, refused at startup unless it reproduces SSH-P2's pinned vector — the ADR 0023 D2 drift guard) with pinned host/user keys, one session channel and one fixed `exec`, TX paced one encrypted ≤192-byte segment per guest ACK. **The gate caught and SSH5 fixed two latent SSH4 wire bugs** that the class-A scripted peers had pinned wrong: the `publickey` request sent the signed `string session_id` prefix on the wire (RFC 4252 §7 signs it but the packet starts at `SSH_MSG_USERAUTH_REQUEST`), and `CHANNEL_EOF` was parsed as 1 byte instead of carrying the RFC 4254 §5.3 recipient channel. Live class-B (Apple silicon VZ, 2026-09-12): `live-ssh-endpoint` **PASS 1/1** (guest `kex-ok`/`pin-ok`/`auth-ok method=publickey`/`channel-open remote=42`, the responder's marker + `exit-status=0` + `eof` + `bye rc=0`; responder log shows KEX, `publickey accepted` and the exec; the seed never appears), `live-ssh-negative` **PASS 3/3** (unknown host pin → `rc=4` with no service request; wrong user key → `AuthRejected rc=5`; tampered MAC → `protocol rc=8`), `live-ssh-nocred` **PASS 1/1** (`MissingCredential rc=5`); no TX clobbering observed across the five runs. Boot default unchanged: responder-adjacent fleet re-run green (`live-ssh-packet`, `live-fetch`, `live-net-tcp` 3/3, `live-net-tcp-rto` 3/3, `live-remote-auth2` 4/4, `live-exec`, `live-httpd`, `live-n1-ping`), `zig build test` 3321/3321, `just verify-portable` green, `inventory-gates --check` green. **Real-OpenSSH interop (issue #1209, 2026-09-12): transport proved, one conformance gap observed.** VZ-NAT guest→host TCP is confirmed dead on this host (while the VM ran, no host interface carried the guest subnet — `vmenet0` inactive, no address — no host route/ARP for 192.168.64.0/24, a synthetic-MAC framework-internal gateway, the macOS firewall OFF, and `sshd` bound `*:2222` at DEBUG3 logging **nothing** while the guest's own `net tcp connect` sat in SYN_SENT). The run therefore went through a new **byte-transparent runner relay** (`--net-tcp-respond <ip>:<port>:relay` + `--net-tcp-respond-relay 127.0.0.1:2222`; no crypto in the runner) to the host's real `/usr/sbin/sshd` (OpenSSH_10.3p1, LibreSSL 3.3.6). Observed: `SSH.BIN tbuddy@10.0.0.2:2222 echo VIRELAI-INTEROP-OK` negotiated `curve25519-sha256` + `ssh-ed25519` + `chacha20-poly1305@openssh.com` (no compression) and **verified the real server's host key against the `SSH/KNOWN_HOSTS` pin** (`ssh: kex-ok`); real sshd then rejected the first encrypted packet (`SSH2_MSG_SERVICE_REQUEST`) with `padding error: need 28 block 8 mod 4` → `SSH2_MSG_DISCONNECT: Packet corrupt` / `message authentication code incorrect`, so userauth, remote stdout and `exit-status` were **NOT** observed (seed absent from every log). Root cause, verified against openssh-portable `packet.c`: OpenSSH pads `chacha20-poly1305` so `packet_length` (the encrypted part after the 4-byte length) is block-aligned, while the client and VSSH aligned `4 + packet_length` (correct for the plaintext KEX path, which is why KEX completed). Fix: follow-up claim **#1210** (client + VSSH realignment, deliberately not smuggled into the evidence change); raw evidence in `artifacts/m51-interop/`. **Claim #1210 fixed it the same day — real interop OBSERVED end to end (2026-09-12).** The alignment is now selected per cipher (`packet.Alignment`): `.plaintext` `(4 + packet_length) % 8 == 0` for every KEX packet, `.aead` `packet_length % 8 == 0` for `chacha20-poly1305@openssh.com` (OpenSSH `ssh_packet_send2_wrapped`), mirrored in VSSH whose startup drift guard also pins the OpenSSH `packet_length=0x48` frame. The same relay run then surfaced two more real-server gaps, both fixed with class-A vectors: `channel.open()` rejected OpenSSH's pre-confirmation `hostkeys-00@openssh.com` GLOBAL_REQUEST (which carries request-specific host-key data — RFC 4254 §4 says ignore it) and treated sender channel id **0** as "unset". Observed in `artifacts/m51-interop/runs/relay-06/` (serial + sshd DEBUG3 + runner stdout; seed absent from every log): `SSH.BIN tbuddy@10.0.0.2:2222 echo VIRELAI-INTEROP-OK` → `ssh: kex-ok` → `pin-ok` → `auth-ok method=publickey` → `channel-open remote=0` → remote stdout `VIRELAI-INTEROP-OK` → `eof` → `ssh: exit-status=0` → `bye rc=0`; sshd logged `Accepted publickey`, the `exec` request and `exit-status 0`. `live-ssh-endpoint` **PASS 1/1**, `live-ssh-negative` **PASS 3/3** and `live-ssh-packet` **PASS** re-ran green with the realigned responder; `zig build test`, `swift build`, `inventory-gates --check` green.
- **M49 — A shell I'd use daily** (umbrella #1127, goal #1065; cards #1128–#1132) — ✅ **SD1–SD5 landed 2026-09-11** (claim #1149). **SD1** `monitor` is a real `SH.BIN`/`TERM.BIN` builtin (a distinct `.monitor` action in the pure shell core): it detaches the front-end, closes `/dev/tty`, and exits; the kernel monitor's `login_console_relinquished` reclaims the raw console, and its ownerless-console fallback also resumes when the login `SH.BIN` pid dies before ever attaching (pure `login_should_resume`, class-A-tested). Class-B `live-sh-monitor` **PASS 1/1** (`shell=sh` boot → typed `monitor` → `virelai-kernel` on the monitor; ordering asserted). **SD2** one startup contract (`user/src/lib/startup.zig`, host-tested): `STARTUP.SH` then the optional host-share `PROFILE.SH`, run by **both** `SH.BIN` and `TERM.BIN` before the first prompt; the kernel monitor's `.virelairc` stays monitor-scope. Class-B `live-sh-profile` **PASS 1/1** (START-UP < PRO-FILE < typed command). **SD3** `user/src/lib/toolbox.zig` — a bounded freestanding busybox-style multicall (`head tail wc grep sort cut test [ printf`, plus shell `read` and a single-line `case`) over pure Stream/Writer/Host seams (13 class-A tests); `SH.BIN`/`TERM.BIN` run it as builtins (so pipes/redirection capture tool output) and `TOOL.BIN` (`user/src/tool.zig`, first argument selects the tool) runs it standalone. The WASM `wc` capstone is deliberately **not** reused (it is a WASM guest for `WASM.BIN`, not an in-process library). Class-B `live-sh-tools` **PASS 1/1** (`exec TOOL.BIN wc -l DATA.TXT` + a sourced share script: head/wc/grep/sort/printf/cut/case/read/`env | grep PWD`). **SD4** editing ergonomics in `lib/tty.zig` (29 tty class-A tests): POSIX backslash-newline continuation, bracketed paste (`ESC [ 200~`/`201~`, literal bytes), Ctrl-L with optional scrollback clear (`ESC [ 3J`), candidate listing when a Tab cycle wraps, `$VAR` completion, and bounded vi/emacs modes (`set -o vi` / `set +o vi`). **SD5** `TERM.BIN` first-class: the kernel presentation grid gained an effective column count + paint-path resize reflow, a scrollback view (PageUp/PageDown, Shift+Up/Down/Home/End), and pointer text selection with highlight; Ctrl+Shift+C copies the selection to the shared clipboard. Class-B `live-term-depth` **PASS 1/1** (pointer drag over the window client area → `tty: copy 7 bytes` → monitor `clip` prints `LINE-00`, ordering asserted) plus class-A reflow/scroll/selection tests. **Boot default unchanged** (must-observe): `live-sh`, `live-sh4`, `live-sh5`, `live-sh-complete`, `live-shell-default`, `live-term` all **PASS** (`live-term`'s `dui` phase was hardened to expect the post-`dui` output instead of racing its 1.5 s tail window); `zig build test`, `just verify-portable`, `zig fmt --check`, `inventory-gates --check` green.
- **Go runtime port — `GOOS=virelai`** (claim #1163 → [ADR 0026](decisions/0026-go-runtime-port.md)) — 🚧 **phase 0a in flight 2026-09-12** (single-thread first target; threads/futex = phase 0b, fault delivery = 0c). A maintained fork of the gc toolchain (the Fuchsia/TamaGo pattern — upstream has no third-party-GOOS mechanism; #35956 declined bare-metal, #73608's GOOSPKG overlay is still open). Fork source of truth lives in-repo: `tools/go/` (5 GOOS-gated overlay files + 6 declarative edits via `apply.sh`; 1.27.1 distribution base). Kernel phase 0a landed (merged via PR #1187): `elf.zig` takes up to 3 PT_LOADs with a new GAP layout (segments at page-aligned declared vaddrs — the Go linker's [R+X][R][RW] shape) beside the original contiguous contract, `exec.zig` maps gap images at declared vaddrs via the aperture machinery, `process.zig` caps lifted (16 mmap regions, 4096 recorded demand pages), `sys_mmap` per-call cap 1 GiB, `sys_getrandom` (the M51 SSH-P1 #1166 slot 72 — this port consumes it), CPACR_EL1.FPEN armed at boot. **0b round 1 in flight (claim #1194)**: per-task uaccess region arrays sized for the mmap caps with a LOUD capacity pre-check (the silent-drop bug class — mmap'd memory invisible to copy_in — is dead, with visibility + ENOMEM regression tests), `just go-toolchain` provisioning recipe, and the **raw-ELF argv card**: the gap loader packs the card-3e argv block (program name prepended) into the writable segment's reserved tail page, the GOOS rt0 stub converts it to a SysV char\* array, `goenvs` builds `runtime.argslice`, and the new `go-args` class-B gate proves the chain (`go-args n=3 [GOARGS.ELF] [alpha] [beta]`). **ADR 0027** (PROPOSED) drafts the phase-0b thread/futex model — slots 73/74, M↔task mapping, the five `proc.go` deltas it retires, and the `go-goroutines` gate shape; no thread kernel code until it is reviewed. Memory = the runtime's sbrk platform over demand-backed `sys_mmap` (contiguity via address hints); nanotime = CNTPCT_EL0 scaled by a boot-time multiplier (EL0-readable counter, no clock syscall); no signals, locks from `lock_sema.go`, sysmon fork-gated off until slots 73/74. Class-B gate `go-hello` (`tools/gate/specs/go-hello.spec`) execs `.build/go/GOHELLO.ELF` (machine prerequisite: `bash tools/go/build-go.sh` — the fork is not a repo artifact) and asserts console + 1 MiB heap growth + a full GC cycle on serial — **PASS 1/1 on VZ 2026-09-12**, clean-room (fresh distribution → apply.sh → toolchain → binary → gate) verified end-to-end; class-A `zig build`/unit tests/BSS budget/fmt/inventory all green. Bring-up lessons are in [ADR 0026](decisions/0026-go-runtime-port.md)'s bring-up record: the runtime needs multi-M semantics from `gcenable` onward (five proc.go deltas behind one `canCreateM` const), prints must stage through the data segment (uaccess does not know the sbrk heap), and the 1.21 MiB of PT_LOAD memory drove the 2 MiB staging bound (ADR 0013 D3.1 amended, budget 13.0 MiB).
- **TABWM polish — issues #1056 / #1058** (follow-up to #1055) — ✅ **done 2026-09-10** (two stacked PRs, one claim #1059): #1056 items — test hardening (`resetForTest()` in every TABWM test); tab navigation + overflow + persistence (Up/Down selection, Enter activates, PageUp/PageDown scroll, scrolled list window with a `^1-15/16v` indicator, `.tabs` host-share persistence re-applied by title); window titles for non-tabapp apps (seat-gated `WMCTL WINDOW_NAME`, cmd 14, resolving the app title or owner process name); #1058 — the true boot clock: the loader's `EFI RuntimeServices.GetTime` runs through the pure `boot/src/efi_time.zig` days-from-civil conversion into the **v3** `HandoffV2.boot_epoch_secs` (version bump, offsets 0..64 frozen) → `timer.wall_epoch()` → the new **ADR 0007 slot 66 `sys_time`** (implemented_count 67), which TABWM prefers over `.clock`/uptime. ADR 0004 D5 + ADR 0007 + ADR 0015 updated. Verified: `zig build test` green (incl. `efi_time` date vectors), `just verify-portable` green, live VZ `live-tabwm` 1/1 + `live-tabwm-fullscreen` 2/2 (serial `tabwm: clock-source kernel`).
...
| **Live in-guest-compiled snake gate** (claim #988) | `bash tools/verify-live-snake.sh` | ✅ **PASS 2026-09-04** | first live RUN consumer of the VL6 GUI surface + the raw `zc.svc(21, &buf)` event-poll seam (ADR 0009): one display-backed boot has ZC.BIN compile the 4-file snake group in-guest (strace-traced source reads, 2048-B cap guard), then SNAKE.ELF runs windowed — ordered markers snake-up → snake-wait → snake-move → snake-over, sys_exit 72 (distinct from the compiler task's 0) — and the host framebuffer captures (ScreenCaptureKit, retina 2×) prove the game's pixels inside the board rect: dark 0x0B1220 board (darkfrac 1.00), green head (0xA7F3D0, 68 hits) and pink food (0xF43F5E, 16 hits) at calibrated thresholds; the pre-window 5 s capture correctly BAD, 3/4 captures OK. Artifacts `live-snake-*` |
| **Live TABWM tab-close seam gate** (claim #1008, ADR 0018 D2) | `bash tools/gate/vgate.sh tools/gate/specs/live-tabwm-close.spec` | ✅ **PASS 2026-09-05** | M42 UX hardening on live VZ VM: one headless boot starts TABWM, execs CALC.BIN (tab-aware full viewport), and INJECTS one pointer click on the active tab's close box (`--pointer-virtio 158,70,c` after `calc: ready`). TABWM routes the click to close_tab → the NEW slot-65 WMCTL_WIN_CLOSE seam (cmd 13); the kernel applies its own `user_close` release — the marker `tabwm: win-close id=2 closed=1` proves the KERNEL (not a hide-only fallback) applied the close, CALC receives the real WIN_CLOSE event (`calc: win_close`) and exits cleanly (`calc: exiting 43`), and `dui` shows the four fixed layers only (the user window released from the kernel registry). Artifacts `live-tabwm-close-*` |
| **Live TABWM unsaved-dialog + Alt-Tab gates** (claim #1011, ADR 0018 round 2) | `bash tools/gate/vgate.sh tools/gate/specs/live-tabwm-unsaved.spec` · `bash tools/gate/vgate.sh tools/gate/specs/live-tabwm-alttab.spec` | ✅ **PASS 2026-09-05** (unsaved 2/2, alttab 1/1) | M42 UX round 2 on live VZ VM (class A green: tabwm 75/75 incl. 11 new r2 tests; `artifacts/2026-09-05-tabwm-unsaved-alttab/`). unsaved spec: two boots — `dui unsaved 2 1` dirties NOTEPAD headless, the injected close-box click is INTERCEPTED (`tabwm: unsaved-dialog id=2`, no close), the Save click drives `tabwm: unsaved-save` → `notepad: saved ok` → `tabwm: win-close id=2 closed=1` → `notepad: win_unsaved` → `notepad: exiting 43` (NOTEPAD treats WIN_UNSAVED as save-and-exit; `notepad: win_close` belongs to the discard path), the Don't Save click drives `tabwm: unsaved-discard` → `notepad: win_close` → `notepad: exiting 43` with `dui: windows=4` and no save marker. alttab spec: one boot, CALC+NOTEPAD tabs, one real `alt-tab` chord → `tabwm: alt-tab id=2`, kernel ` alt_tab=` counter moved, `dui: windows=6 focused=2` (CALC holds kernel focus). Artifacts `live-tabwm-unsaved-*` / `live-tabwm-alttab-*` |
