...
17. **Milestone six, card G1 — virtio-gpu transport + framebuffer (DONE
    ...
    **What's next: card G4 — virtio input (keyboard + pointer)** so
    keystrokes come from the screen side (virtio-input transport, DID
    0x1052 — [inferred] until observed; bounded event FIFO feeding Road
    Pops' line editor), then G5 **Driving Award**. Plan
