...
| Thirty-four — FAT-free storage (proposed) | Host file channel (HF1–HF7): the guest userland filesystem becomes a macOS folder over the custom-virtio device; FAT32 shrinks to firmware-parsed boot infra | 🔄 in progress (issue #727; `docs/host-file-channel-scoping.md` — HF1–HF5 done: queue-5 wire + `vf ls`/`vf cat` (PR #745, claim 7710), mutation verbs host-verified (PR #747, claim 9459), app delivery — exec from the share, desktop manifest re-pointed, drop-`.ELF`-and-exec with no image rebuild (PR #749, claim 7599), user-data migration — `/host` READ-WRITE, one-time `/data`→share boot migration, every persistence consumer + gate re-pointed with host-disk verification (PR #792, claim 3082); **HF6 FAT removal done** (PR #806, claim 4780 — `fat.zig`/`esp.zig`/`virtio_blk.zig`/`migrate.zig` deleted, DATA partition gone, image = single boot volume, gate fleet on ONE shared read-only image with per-gate private shares); HF7 CLONE dedup remains; known flake: `verify-live-exec` corrupts exception frames ~80% of boots (issue #803) |
| Thirty-five — WASM core interpreter | Guest-side bounded wasm-core interpreter (`WASM.BIN`, an EL0 app — modules are *data* over the M34/HF4 file channel, `env.*` imports map to ADR 0007 syscalls, no WASI): W1b integer core + W2 live proof, W3 frozen 30-import `env.*` surface (PR #807), **W4 core completeness** — f32/f64 + conversions/reinterpret with the W1 trap discipline (div-by-zero → ±inf; plain trunc traps, saturating trunc clamps), sign-extension proof fixture, bulk-memory 0xFC 8–11 justified by a clang probe (runtime-size memcpy/memset lower to `memory.copy`/`memory.fill`); the named C float utility `tests/floatapp.c` pins byte-exact 590-byte in-guest output (claim 7395) | 🔄 W4 done 2026-09-02 (claim 7395; live gate PASS: five float opcode families byte-exact, exit 590) — [GH milestone 22](https://github.com/drawmeanelephant/DipshitOS/milestone/22): **5/6 closed, W5 `wc` capstone (#766) open** — [scoping](wasm-core-scoping.md) |
...
- **M35 — WASM core interpreter** ([GH milestone 22](https://github.com/drawmeanelephant/DipshitOS/milestone/22); scoping: [`docs/wasm-core-scoping.md`](wasm-core-scoping.md)) — W1a–W4 landed; **W5 (`wc` capstone, #766) is the last card**, whose prereq W4 (claim 7395) is complete.
