...
- **Terminal (vt) seam — issue #1072 (M44), foundation landed 2026-09-10** (claim #1073): ADR 0020 fixes the ABI decision — a terminal is a **device file** (`/dev/tty` via the existing `sys_file_open`/read/write slots; **no new syscall**) backed by a pure, host-tested `kernel/src/terminal.zig` (bounded output ring + input queue + exclusive front-end attach); `kernel/src/file_table.zig` routes `/dev/tty` to a process's controlling terminal. Boot default unchanged (the kernel monitor keeps the raw serial console; ADR 0020 D4). Class-A: `zig build test` 2360/2360, `just verify-portable` green; live `live-tabwm` 1/1. Next tranche: the serial front-end pump + the EL0 pilot.
