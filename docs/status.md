...
| **Live EL0/SVC boundary gate** | `bash tools/verify-live-userspace.sh` | ✅ **PASS 2026-08-10** | claim 8215 regression, 1/1 on the syscall branch: two sequenced pings prove return to EL0; the payload waits for a timer-only witness before cooperative yield, and the shell-side evidence follows real timer preemption |
| **Live syscall-table gate** | `bash tools/verify-live-svc.sh` | ✅ **PASS 2026-08-10** | claim 3594, 1/1: one complete LF-terminated EL0 write; timer IRQ before yield/exit; exact single snapshot `ping=2`, `write=1`, `yield=1`, `exit=1`; non-returning status-7 exit and one post-exit `rx-svc-ok` reply. Corrected 3,987-byte serial evidence supersedes the earlier replaying FIFO/unterminated-line artifacts. |
...
- [x] At least ten commands work (22 commands, host-tested; the registry grew 20 → 21 with claim 5844's `pci` and → 22 with claim 5275's `tasks`; real command output observed post-exit via the NVRAM channel, claim 0015, and live post-MMU via claim 1517).
