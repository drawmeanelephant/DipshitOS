...
| **M61 — Guest self-test** | ADR 0031 landed (#1381); M61b `GOSELF.ELF` + `go-selftest` VZ-green (#1382, PR #1392); M61c intake (#1383) reads the host-seeded `IN/` fixtures and the host byte-compares the guest's copies; M61d (#1384) adds the file-ABI pack (`file-roundtrip` / `file-truncate` / `file-delete` / `file-list`) with a receipt and a read-back copy per case, byte-compared on macOS; M61e (#1385) adds the `window` receipt — `sys_win_query`'s read-back of the kernel's window record, held to the kernel's own `open: id=` line and TABWM's `tab-switch id=`, so a case that restated its request (or named a window the kernel does not have) fails the gate; M61f (#1386) adds the `share-equals`/`share-contains` assert kinds (the first amendment to SPEC.md since M40 GF2), piloted in `go-selftest.spec` so a spec can byte-compare the share's own files without a `python` block. #1391 is fixed kernel-side (ADR 0032 — `copy_out` resolves its destination page in the process's own root), the read helper's workaround is deleted and the intake case is its regression test. **M61 complete.** | #1380 |
