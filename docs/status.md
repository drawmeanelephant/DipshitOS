...
| **Tab-strip chrome red (observed, not M66c)** | `live-tabstrip`, and `live-tabclick` runs B/C, fail on this tree with EITHER tab host — the Go client or a shipping Zig app (`TOP.BIN`) — so the deletion is not the cause: the strip row paints its cell background but none of the per-tab decorations (dividers, active underline, label ink, × glyph), and the ×/drag paths never detach. Found while retargeting #1485; root cause not chased. | |
| **M60 leftovers** | Policy recorded; `EDIT.BIN`, `FILE.BIN`, `CALC.BIN`, and (M66c/#1485) `NOTEPAD.BIN` gone. Remaining Zig EL0 (TABWM, SH, TLS/SSH helpers) deletes one binary per card when its Go successor is VZ-green. | |
