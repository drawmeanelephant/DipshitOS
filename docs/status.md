...
| 60 | Starve Zig EL0 (#1297) | No new `user/src/*.zig` apps (ADR 0030). First leftover deleted: `EDIT.BIN` → `GOEDIT.ELF` (gate `go-edit`). Further Zig EL0 deletions are later cards. TLS stays `FETCHS.BIN` / `lib/tls`; SSH stays `SSH.BIN`. | 🔄 2026-09-16 (policy + first deletion) |
...
| **M60 leftovers** | Policy recorded; `EDIT.BIN` gone. Remaining Zig EL0 (CALC, NOTEPAD, FILE, TABWM, SH, TLS/SSH helpers) deletes one binary per card when its Go successor is VZ-green. | |
