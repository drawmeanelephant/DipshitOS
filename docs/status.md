...
| Thirty-three — seam B (proposed) | Full pixel ownership: cross-process shared-anon mmap, apps render into their own buffers, WM composes one final present | 🔄 in progress (ADR 0016 ACCEPTED; SB1 contract done 2026-08-30 claim 7418, SB2 capability done 2026-08-31 claim 8878 with a live headless VZ gate, SB3 surface handoff done 2026-08-31 claim 3633 — migrated apps bind a window to a shared surface and the WM reads the bytes RO, SB4 damage tracking done 2026-08-31 claim 2382 — rect-granular per-surface damage delivered to the WM via the COMPOSITE_TICK arg1 mask, one rect writes -> one rect repaints (live VZ gate, dui last= column); [`docs/march-m33-seam-b-pixel-ownership.md`](march-m33-seam-b-pixel-ownership.md); [GH milestone 17](https://github.com/drawmeanelephant/DipshitOS/milestone/17)) |
| Thirty-four — FAT-free storage (proposed) | Host file channel (HF1–HF7): the guest userland filesystem becomes a macOS folder over the custom-virtio device; FAT32 shrinks to firmware-parsed boot infra | 🔄 in progress (issue #727; `docs/host-file-channel-scoping.md` — HF1–HF4 done: queue-5 wire + `vf ls`/`vf cat` (PR #745, claim 7710), mutation verbs host-verified (PR #747, claim 9459), app delivery — exec from the share, desktop manifest re-pointed, drop-`.ELF`-and-exec with no image rebuild (PR #749, claim 7599); HF5 user-data migration, HF6 FAT removal, HF7 CLONE dedup remain) |
