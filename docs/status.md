...
| 65 | Go threads (ADR 0027) | Slot 73/74 host tests (M65a/b #1472/#1473). M65c retires leftover 0a `proc.go` deltas on apply; `go-goroutines`/`go-stress` use compiled default GOMAXPROCS. M65d `max_tasks`. | 🔄 2026-09-18 |
...
| **M65 — Go threads (ADR 0027)** | M65a/b landed (#1472/#1473). M65c: leftover 0a `proc.go` reverse + default GOMAXPROCS in thread gates. M65d `max_tasks`. | #1433 |
