...
16. **Milestone five, card N1 — virtio-net transport + TX (the roadmap's
    ...
    now satisfied).** Plan: `docs/m5-net-tx-prompt.md`. Runner gains a
    flag-gated `--net` mode (file-handle attachment; the default VM stays
    byte-identical for the 28-gate sweep); guest `kernel/src/virtio_net.zig`
    (expect DID 0x1041, post-exit re-arm, MAC via `VIRTIO_NET_F_MAC`,
    bounded BSS frame staging); `net`/`netsend` monitor commands; class-B
    gate `tools/verify-live-net-tx.sh` (host receives a known frame
    byte-exact). RX (N2), ARP (N3), IPv4 (N4) are later cards.
