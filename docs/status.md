...
| **M79e — nav kinds 9/10** | Code-complete on `agent/buffy/m79e-nav-kinds`: the seat now serves nav-declare/nav-poll, `Ctrl+Shift+[` / `]` steps per-tab history, and GOFILES is the first live adopter. Blocked on a `hid.go` Touches overlap with #1747. | #1708 |
