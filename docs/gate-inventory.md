...
| `verify-vz` | B | aggregate | no | no | yes | `just verify-vz` — serial takeover + bad-handoff + marker + nvram-console + host-console + live-transcript + live-fs + live-exceptions + live-timer + live-tasks + live-userspace + live-svc + live-uaccess + live-addrspaces + live-lifecycle + live-exec + live-procs + live-sleep + live-entropy + live-reboot-shutdown (Apple silicon only) |
