...
| `live-ipc` | B | gate | yes | no | yes | `bash tools/verify-live-ipc.sh` — **claim 5965** (milestone-four follow-on 3, card 3f): IPC — distinct processes EXCHANGE DATA on real VZ — `exec PEER.BIN` + `exec COUNTER.BIN 1` back to back (the counter's target pid is argv[0] — card 3e's entry contract): the counter's `ipc: ping N` sends (sys_ipc_send, slot 5) and the peer's byte-exact `peer: got ping N` echoes (sys_ipc_recv, slot 6) INTERLEAVE across the whole log (send N → echo N, every send echoed, first echo after the first send — end-to-end data flow between TWO never-exiting processes through the bounded per-process kernel mailbox, 4×64 B BSS rings); the phase-2 `mbox` snapshot shows PEER.BIN `pending=0 sent=1 recv=1` (the bounded ring DRAINED — the sent − recv == pending ≤ 1 invariant) and COUNTER.BIN `pending=0 sent=0 recv=0`; both processes are `state=running` at the final `procs` and neither ever exits; a fifth exec is `exec: no free scheduler pool slot` (7/7 pool — card 3g re-derived the refusal); the shell stays responsive. Two scripted phases (`--script` after the boot payload exits, `--script2` after `peer: got ping 1` — the flow is established before the drain snapshot) |
...
| `verify-vz` | B | aggregate | no | no | yes | `just verify-vz` — serial takeover + bad-handoff + marker + nvram-console + host-console + live-transcript + live-fs + live-gfs + live-exceptions + live-timer + live-tasks + live-userspace + live-svc + live-uaccess + live-addrspaces + live-lifecycle + live-exec + live-procs + live-concurrent + live-args + live-long-lived + live-kill + live-ipc + live-scale + live-procs-syscall + live-sleep + live-entropy + live-reboot-shutdown (Apple silicon only) |
