...
| 2 | **Syscall ABI + dispatch table.** Freeze the numbered contract, runtime-built dispatch table, argument/result plumbing, errors, ADR, portable coverage, and a dedicated live SVC gate. | ⬜ not started | Not yet claimed; card: [`docs/m3-syscall-abi-prompt.md`](m3-syscall-abi-prompt.md) | Depends on step 1, which is now landed. Extend the merged exception/SVC seam rather than adding a parallel entry path. The ABI card owns its contract corrections and evidence. |
